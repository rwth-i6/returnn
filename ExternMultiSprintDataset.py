...
class ExternMultiSprintDataset3(SprintDataset):
  """
  ...
  def __init__(self, sprintTrainerExecPath, sprintConfigStr, partitionEpoch=1, targetlabel=None, *args, **kwargs):
    """
    :type sprintTrainerExecPath: str
    :type sprintConfigStr: str
    """
    super(ExternMultiSprintDataset3, self).__init__(*args, **kwargs)
    ...
    self.targetlabels = targetlabel      # If set, first "classes" will be replaced by the first string, all other targets will be added as dummies (i.e. -1).
    self._num_seqs = None
    self.child_pid = None
  ...
  def _exit_child(self, wait_thread=True):
    if self.child_pid:
      ...
      if self._join_child(wait=False, expected_exit_status=expected_exit_status) is False:  # Not yet terminated.
        ...
        if interrupt:
          print >> log.v5, "ExternMultiSprintDataset3: interrupt child proc %i" % self.child_pid
  ...
  def _start_child(self, epoch):
    ...
    print >>log.v5, "ExternMultiSprintDataset3: epoch", epoch, "exec", args
    ...
    if pid == 0:  # child
      try:
        ...
        print >> log.v1, "ExternMultiSprintDataset3: Error when starting Sprint %r." % args
    ...
    try:
      ...
      print >> log.v1, "ExternMultiSprintDataset3: Sprint child process (%r) caused an exception." % args
      sys.excepthook(*sys.exc_info())
      raise Exception("ExternMultiSprintDataset3 Sprint init failed")

    self.reader_thread = Thread(target=self.reader_thread_proc, args=(pid, epoch,),
                                name="ExternMultiSprintDataset3 reader thread")
    ...
    try:
      ...
      while not self.python_exit:
        ...
        with self.lock:
          ...
          if dataType == "data":
            segmentName, features, targets_ = args
            
            if self.targetlabels: #relabel the target
              targets = { self.targetlabels[0]: targets_["classes"]}
              for label in self.targetlabels[1:]:
                targets[label] = numpy.array([]) #-1*numpy.ones_like(targets_["classes"])
            else:
              targets = targets_
      ...
      if self.seq_list_file:
        try:
          ...
          print >> log.v5, "ExternMultiSprintDataset3: error when removing %r: %r" % (self.seq_list_file, e)
        finally:
          self.seq_list_file = None
      ...
      print >> log.v5, "ExternMultiSprintDataset3 finished reading epoch %i" % epoch
      ...
      try:
        print >> log.v1, "ExternMultiSprintDataset3 reader failed"
        sys.excepthook(*sys.exc_info())
        print ""
      finally:
  ...
  def init_epoch(self, epoch=None, seq_list=None):
    ...
    with self.lock:
      ...
      super(ExternMultiSprintDataset3, self).init_seq_order(epoch=epoch, seq_list=seq_list)
    self._exit_child()
  ...
  def setDimensions(self, inputDim, outputDim):
    ...
    if outputDim > 0:
      if self.targetlabels:
        self.num_outputs[self.targetlabels[0]] = [outputDim, 1]
      else:
