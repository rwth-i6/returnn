...
def main():
  ...
  if args.config:
    ...
    lr = LearningRateControl.load_initial_from_config()
    ...
    assert False, f"should not get here with {args}"

  epochs = sorted(lr.epoch_data.keys())
  if not epochs:
    print("Error: no epochs found")
    sys.exit(1)
  print(f"Loaded epochs {epochs[0]}..{epochs[-1]}")
  ...
  print(f"Using key {key} (via epoch {last_epoch_with_error_info})")
  ...
  if len(epochs_) == len(epochs):
    ...
    print(f"Epochs missing the ky: {missing_epochs}")
  assert epochs_
  epochs_.sort()

  for value, ep in epochs_[:args.n]:
    errors = lr.epoch_data[ep].error
    print(f"Epoch {ep}, {key} {value}, " + ", ".join(f"{k} {v}" for k, v in errors.items() if k != key))
