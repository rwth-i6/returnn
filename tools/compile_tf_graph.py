...
class SubnetworkRecCellSingleStep(_SubnetworkRecCell):
    ...
    def _maybe_delay_tiled(self, state_var, output):
        ...
        if rec_layer.construction_state == rec_layer.ConstructionState.GetSources:
            ...
            with tf.control_dependencies([state_var.init_op()]):
                x = tf.identity(
                    output.placeholder,
                    name="state_var_before_loop_depend_on_init__%s"
                    % tf_util.get_valid_scope_name_from_str(state_var.name),
                )
    ...
    def get_parent_dim_tag(self, dim_tag):
        ...
        dim_tag_ = Dim(
            kind=dim_tag.kind,
            description=dim_tag.description + "_base_state_var",
            dimension=None,
            dyn_size_ext=dim_tag_dyn_size_ext,
        ...
        for layer_name in layers_with_state:
            ...
            if not choice_deps:
                ...
                print(
                    ...
                    % (layer_name, choice_deps, prev_frame_deps),
                    file=log.v4,
                )
                if source_deps:
                    raise NotImplementedError(
                        "Delayed layers with source dependencies (%s) not supported yet" % source_deps
                    )
        ...
        class _LayerStateHelper:
            def __init__(self, layer_name, prefix):
                ...
                if layer_name in init_extra:
                    ...
                    data_shapes["extra"] = nest.map_structure(
                        lambda s: None, init_extra[layer_name]
                    )  # should be batch-major
                assert isinstance(rec_layer, RecStepByStepLayer)
                self.state_vars = rec_layer.create_state_vars_recursive(
                    name_prefix="%s/%s" % (prefix, layer_name), initial_values=initial_values, data_shapes=data_shapes
                )
        ...
        with tf.name_scope("delayed_state_update"):
            ...
            def _delayed_state_update():
                self._construct_custom(
                    ...
                    cur_state={
                        layer_name: layers_prev[layer_name].get_reads_once() for layer_name in layers_cur_iteration
                    },
        ...
        def _add_predefined_layer(layer_name, state_dict, prev):
            ...
            try:
                layer = self.layer_data_templates[name].copy_as_prev_time_frame(
                    prev_output=state_dict.get("output", None), rec_vars_prev_outputs=state_dict.get("extra", None)
                )
    ...
    def _construct(self, prev_outputs, prev_extra, i, data=None, inputs_moved_out_tas=None, needed_outputs=("output",)):
        ...
        for layer in self.parent_net.get_root_network().get_all_layers_deep():
            ...
            all_dyn_parent_dim_tags.update(
                [dim for dim in layer.output.dim_tags if dim.dimension is None and not dim.is_batch_dim()]
            )
        ...
        super(SubnetworkRecCellSingleStep, self)._construct(
            ...
            i=i,
            data=data,
            inputs_moved_out_tas=inputs_moved_out_tas,
    ...
    * "next_step_op: str. op. the update op for all state vars.
    ...
    class StateVar:
        ...
        def __init__(self, parent, name, initial_value, data_shape):
            ...
            self.var_data_shape = (
                data_shape.copy_as_batch_major() if data_shape.batch_dim_axis is not None else data_shape
            )
            del data_shape
            ...
            with helper_variable_scope():
                self.var = tf_compat.v1.get_variable(
                    name=tf_util.get_valid_scope_name_from_str(name), initializer=zero_initializer, validate_shape=False
                )  # type: tf.Variable
        ...
        def init_op(self):
            ...
            self._init_op = tf_compat.v1.assign(
                self.var, self.var_initial_value, name="init_state_var_%s" % self.name
            ).op
            return self._init_op
    ...
    def __init__(
        ...
        unit,
        axis,
        rec_step_by_step_opts,
        reverse_stochastic_var_order=False,
        **kwargs,
    ):
        """
        ...
        if axis and sources and sources[0].base_layer.output.have_dim_tag(axis):
            ...
            if output.have_dim_tag(base_axis):
                output = output.copy_template_replace_dim_tag(
                    axis=output.get_axis_from_description(base_axis), new_dim_tag=axis
                )
    ...
    def get_parent_tile_multiples(self):
        ...
        with tf_util.reuse_name_scope("parent_tile_multiples", absolute=True):
            ...
            with tf.control_dependencies(
                [
                    tf.Assert(
                        tf.equal(loop_batch_dim % base_batch_dim, 0),
...
def main(argv):
    ...
    argparser.add_argument(
        "--epoch", type=int, default=None, help="specific epoch to construct, use for dynamic network definitions"
    )
    ...
    if args.epoch and "get_network" in config.typed_dict:
        ...
        raise NotImplementedError(
            "Compiling a network at a specific epoch using pre-train logic is not implemented yet"
        )
    else:
