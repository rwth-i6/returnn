...
class Nadam(Optimizer):
  ...
  def __init__(self, params,
               lr=1e-3, betas=(0.9,0.999), eps=1e-8, nadam_decay=0.004,
               weight_decay=0.0001,
               gradient_scale=1.0,
               gradient_l2_norm=0,
    ...
    defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay,
                    nadam_decay=nadam_decay, gradient_scale=gradient_scale,
  ...
  def _get_average_grad_norm(self, group):
    total_grad_norm = 0
    n_total_params = 0
    ...
    for group in self.param_groups:
      ...
      for p in group['params']:
        ...
        if group['gradient_clip'] > 0.0:
          grad = torch.clamp(grad, min=-group['gradient_clip'], max=group['gradient_clip'])
