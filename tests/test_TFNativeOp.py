...
def dump_info():
    ...
    if hasattr(sys, "getdlopenflags") and hasattr(sys, "setdlopenflags"):
        ...
        print(
            "Cur flags: %r, RTLD_GLOBAL is set: %r" % (sys.getdlopenflags(), sys.getdlopenflags() & ctypes.RTLD_GLOBAL)
        )
    ...
    for p in tf_util.CudaEnv._cuda_path_candidates():
        print(
            "CUDA path candidate %s, lib dir %s, valid %s"
...
def test_LstmLowMem_bwd_simple_1():
    ...
    vDc2 = (1.0 - vgc * vgc) * vgates * vDh1 + vDd
    vDcellIn_in = (1.0 - vcellIn * vcellIn) * vgates * vDc2
    vDinpGate_in = (1.0 - vgates) * vgates * vcellIn * vDc2
...
def lstm_step_op(x_t, h_tm1, c_tm1, mask_t, W_f, W_r, b, n_batch, n_in_dim, n_cells):
    ...
    h_t = h_t * mask_t_c + h_tm1 * (1.0 - mask_t_c)
    c_t = c_t * mask_t_c + c_tm1 * (1.0 - mask_t_c)
    h_t.set_shape(tf.TensorShape((n_batch, n_cells)))
    c_t.set_shape(tf.TensorShape((n_batch, n_cells)))
    return h_t, c_t


def pure_tf_unrolled_lstm(
    x, h_0, c_0, mask, W_f, W_r, b, n_time, n_batch, n_in_dim, n_cells, start=0, step=1, name="ref_lstm"
):
    """
...
def wrap_lstm_slice_start_step(
    op, x, h_0, c_0, mask, W_f, W_r, b, n_time, n_batch, n_in_dim, n_cells, start, step, name
):
    """
...
def native_lstm2(
    x, h_0, c_0, mask, W_f, W_r, b, n_time, n_batch, n_in_dim, n_cells, start=0, step=1, name="native_lstm2"
):
    """
    :param tf.Tensor x: (n_time, n_batch, n_in_dim)
    :param tf.Tensor h_0: (n_batch, n_cells)
...
def test_native_lstm2_impl():
    kwargs = lstm_kwargs()
    check_lstm_ops(
        op1=pure_tf_unrolled_lstm, op2=native_lstm2, name1="ref_lstm", name2="native_lstm2", rtol=1e-6, **kwargs
    )
...
def lstm_grad_kwargs():
    ...
    def gen(shape, offset, factor=3.0):
...
def wrap_lstm_grad(op, x, h_0, c_0, dy, dd, mask, W_f, W_r, b, n_time, n_batch, n_in_dim, n_cells, start, step, name):
    ...
    y.set_shape(tf.TensorShape((n_time, n_batch, n_cells)))
    d.set_shape(tf.TensorShape((n_batch, n_cells)))
    dx, dh0, dc0, dWf, dWr, db = tf.gradients(ys=[y, d], grad_ys=[dy, dd], xs=[x, h_0, c_0, W_f, W_r, b])
...
def check_lstm_grad_ops_single(op1, op2, name1, name2, dy, dd, rtol=5e-5, exclude=(), **kwargs):
    ...
    (
        ...
        vdb2,
        vmask,
    ) = session.run([y1, d1, y2, d2, dx1, dh01, dc01, dWf1, dWr1, db1, dx2, dh02, dc02, dWf2, dWr2, db2, mask_bc])
    step = kwargs["step"]
    if step >= 0:
        start_ = kwargs["start"]
    else:
...
def check_lstm_grad_start_step(op, name, **kwargs):
    ...
    check_lstm_grad_ops(op1=op, name1=name, op2=wrapped_lstm_op, name2=name2, **kwargs)
...
def test_native_lstm2_grad_start_step():
    kwargs = lstm_grad_kwargs()
    check_lstm_grad_start_step(op=native_lstm2, name="native_lstm2", rtol=1e-5, **kwargs)


def test_native_lstm2_grad():
    kwargs = lstm_grad_kwargs()
    check_lstm_grad_ops(
        op1=pure_tf_unrolled_lstm, name1="ref_lstm", op2=native_lstm2, name2="native_lstm2", rtol=1e-5, **kwargs
    )
...
def pure_tf_chunk(x, index, chunk_size, chunk_step):
    ...
    x_win = windowed_nd(
        x, window_size=chunk_size, window_left=0, time_axis=0, new_window_axis=0
    )  # (chunk_size,time,batch,dim)
    ...
    i_win = windowed_nd(
        index, window_size=chunk_size, window_left=0, time_axis=0, new_window_axis=0
    )  # (chunk_size,time,batch)
...
def pure_tf_unchunk(x, index, chunk_size, chunk_step, n_time, n_batch):
    ...
    factors = where_bc(tf.equal(chunk_idx, 1.0), 1.0 / tf.squeeze(dxn, axis=2), 0.0)
...
def test_fast_viterbi():
    ...
    alignment, obs_scores = fast_viterbi(
        ...
        am_seq_len=tf.constant(numpy.array([seq_len] * n_batch, dtype="int32")),
        edges=tf.constant(edges),
        weights=tf.constant(weights),
        start_end_states=tf.constant(start_end_states),
    )
...
def test_fast_viterbi_rnd():
    ...
    alignment, scores = fast_viterbi(
        am_scores=tf.constant(am_scores),
        am_seq_len=tf.constant(am_seq_len),
        edges=tf.constant(edges),
        weights=tf.constant(weights),
        start_end_states=tf.constant(start_end_states),
    )
...
def test_ctc_viterbi_loss():
    ...
    loss = ctc_loss_viterbi(
        logits=logits,
        logits_seq_lens=x_seq_len,
        logits_time_major=True,
        targets=targets,
        targets_seq_lens=targets_seq_len,
    )
...
def test_optimal_completion_edit_distance_per_successor():
    ...
    for i in range(n_batch):
        ...
        for j in range(num_classes):
            tf_res = _naive_optimal_completion_edit_distance(
                list(a_np[i, : a_len_np[i] - 1]) + [j], b_np[i, : b_len_np[i]]
            )
...
def test_blocksparse_simple():
    ...
    w = tf_compat.v1.get_variable(
        "w", bsmm.w_shape, dtype=tf.float32, initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=3)
    )
...
def _run_rnnt(acts, labels, input_lengths, label_lengths, expected_costs, expected_grads, blank):
