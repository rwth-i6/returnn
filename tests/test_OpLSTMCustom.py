...
def test_bwd_pass_compatible_with_OpLSTM():
  ...
  f = theano.function(inputs=[Z, X, c, i, W_re], outputs=[DZ, DW_re])
  g = theano.function(inputs=[Z, W_re, c, i], outputs=[DZ2, DW_re2])
  ...
  vals = f(Z_val, X_val, c_val, i_val, W_re_val)
  DZ_val, DW_re_val = (numpy.asarray(vals[0]), numpy.asarray(vals[1]))
  vals2 = g(Z_val, W_re_val, c_val, i_val)
