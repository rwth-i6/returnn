...
def test_RecLayer_loss_subsubnet():
  with make_scope() as session:
    config = Config()
    n_in, n_out = 2, 3
    net_dict = {
      'output': {
        'class': 'rec',
        'from': 'data:classes',
        'optimize_move_layers_out': False,  # just to trigger the potential bug
        'unit': {
          'output': {
            'class': 'subnetwork',
