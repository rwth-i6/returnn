...
class CachedDataset(Dataset):
    ...
    def initialize(self):
        ...
        if self.cache_byte_size_limit_at_start > 0:
            ...
            self.definite_cache_leftover = (
                temp_cache_size_bytes if self.num_seqs_cached_at_start == self.num_seqs else 0
            )
            self.cache_num_frames_free = temp_cache_size_bytes // self.nbytes

            print(
                "cached %i seqs" % self.num_seqs_cached_at_start,
    ...
    def init_seq_order(self, epoch=None, seq_list=None, seq_order=None):
        ...
        if seq_order is not None:
            ...
            seq_index = self.get_seq_order_for_epoch(
                epoch, self._num_seqs, lambda s: self._get_seq_length_by_real_idx(s)[0]
            )
        ...
        if (
            ...
            if not self._seq_index_inv:
                self._seq_index_inv = dict(
                    zip(self._seq_index, range(len(self._seq_index)))
                )  # hdf seq idx -> seq_index idx
    ...
    def _init_alloc_intervals(self):
        ...
        self.alloc_intervals = [
            ...
            (
                self.num_seqs,
                self.num_seqs,
                numpy.zeros([1] + self.get_data_shape("data"), dtype=self.get_data_dtype("data")),
            ),
        ]
                ...
                (
                    ...
                    numpy.concatenate(
                        [
                            xc,
                            numpy.zeros(
                                [self._seq_start[value[1]][0] - self._seq_start[ci][0]] + self.get_data_shape("data"),
                                dtype=self.get_data_dtype("data"),
                            ),
                        ]
                    ),
            ...
            self.alloc_intervals.insert(
                ...
                (
                    ...
                    numpy.concatenate(
                        [
                            numpy.zeros(
                                [self._seq_start[ni][0] - self._seq_start[value[0]][0]] + self.get_data_shape("data"),
                                dtype=self.get_data_dtype("data"),
                            ),
                            xc,
                        ]
    ...
    def _remove_alloc_interval(self, pos, value):
        ...
        if value[0] == ci and value[1] == ni:
            ...
            self.alloc_intervals.insert(
                pos, (value[1], ni, xi[self._seq_start[value[1]][0] - self._seq_start[ci][0] :])
            )
            del self.alloc_intervals[pos + 1]
            return 0
        elif value[1] == ni:
            self.alloc_intervals.insert(
                pos, (ci, value[0], xi[: self._seq_start[value[0]][0] - self._seq_start[ci][0]])
            )
            del self.alloc_intervals[pos + 1]
            return 0
        else:
    ...
    def _modify_alloc_intervals(self, start, end, invert):
        ...
        if self.alloc_intervals[0][0] != 0:
            self.alloc_intervals.insert(
                0, (0, 0, numpy.zeros([1] + self.get_data_shape("data"), dtype=self.get_data_dtype("data")))
            )
        if self.alloc_intervals[-1][1] != self.num_seqs:
            self.alloc_intervals.append(
                (
                    self.num_seqs,
                    self.num_seqs,
                    numpy.zeros([1] + self.get_data_shape("data"), dtype=self.get_data_dtype("data")),
