...
class TimitDataset(CachedDataset2):
    ...
    def __init__(
        ...
        feature_window_len=0.025,
        feature_step_len=0.010,
        with_delta=False,
        norm_mean=None,
        norm_std_dev=None,
    ...
    def _preload(self):
        ...
        while True:
            ...
            if cur_len != last_print_len and time.time() - last_print_time > 10:
                print("%r: loading (%i/%i loaded so far)..." % (self, cur_len, len(self._seq_tags)), file=log.v3)
    ...
    def _get_phone_seq(self, seq_tag):
        ...
        while True:
            ...
            if cur_len != last_print_len and time.time() - last_print_time > 10:
                print(
                    "%r: waiting for %r, idx %i (%i/%i loaded so far)..."
                    % (self, seq_tag, idx, cur_len, len(self._seq_tags)),
                    file=log.v3,
                )
                last_print_len = cur_len
                last_print_time = time.time()
    ...
    def _demo_audio_play(self, audio, sample_rate):
        ...
        chunk_size = 1024
        stream = p.open(
            format=pyaudio.paFloat32, channels=1, rate=sample_rate, frames_per_buffer=chunk_size, output=True
        )
        while len(audio) > 0:
    ...
    def _collect_single_seq(self, seq_idx):
        ...
        audio_feature_extractor = ExtractAudioFeatures(
            ...
            num_feature_filters=self._num_feature_filters,
            with_delta=self._with_delta,
            window_len=self._feature_window_len,
            step_len=self._feature_step_len,
            norm_mean=self._norm_mean,
            norm_std_dev=self._norm_std_dev,
            random_permute=self._random_permute_audio,
            random_state=self._random,
        )
        mfccs = audio_feature_extractor.get_audio_features(audio=audio, sample_rate=sample_rate, seq_name=seq_tag)
...
class BlissDataset(CachedDataset2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(
            "%s: Loaded %r, num seqs: %i, elapsed: %s"
            % (self.__class__.__name__, path, len(self._seqs), hms_fraction(time.time() - start_time)),
            file=log.v3,
        )
...
class LibriSpeechCorpus(CachedDataset2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_zip:
            ...
            self._zip_files = {
                os.path.splitext(os.path.basename(fn))[0]: zipfile.ZipFile(fn) for fn in zip_fns
            }  # e.g. "train-clean-100" -> ZipFile
    ...
    def init_seq_order(self, epoch=None, seq_list=None, seq_order=None):
        ...
        if self.epoch_wise_filter:
            ...
            for (ep_start, ep_end), value in sorted(self.epoch_wise_filter.items()):
                ...
                if ep_start <= epoch <= ep_end:
                    ...
                    if opts.get("subdirs") is not None:
                        ...
                        self._seq_order = [
                            idx for idx in self._seq_order if self._reference_seq_order[idx][0] in subdirs
                        ]
                    ...
                    if opts.get("use_new_filter"):
                        ...
                        self._seq_order = EpochWiseFilter.filter_epoch(
                            opts=opts,
                            debug_msg_prefix="%s, epoch %i. " % (self, epoch),
                        ...
                        if opts.get("max_mean_len"):
                            ...
                            print(
                                ...
                                % (
                                    self,
                                    epoch,
                                    float(numpy.mean(seqs[:, 0])),
                                    float(numpy.mean(seqs[:num, 0])),
                                    max_mean_len,
                                    len(seqs),
                                    num,
                                ),
            ...
            if any_filter:
                print(
                    "%s, epoch %i. Old num seqs %i, new num seqs %i." % (self, epoch, old_num_seqs, self._num_seqs),
                    file=log.v4,
                )
            else:
    ...
    def _collect_single_seq(self, seq_idx):
        ...
        return DatasetSeq(features=features, targets=targets, seq_idx=seq_idx, seq_tag=seq_tag)
