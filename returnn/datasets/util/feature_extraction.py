    ...
    def get_audio_features(self, audio, sample_rate, seq_name=None):
        ...
        if self.random_permute_opts and self.random_permute_opts.truth_value:
            audio = _get_random_permuted_audio(
                audio=audio, sample_rate=sample_rate, opts=self.random_permute_opts, random_state=self.random_state
            )
        ...
        if self.with_delta:
            ...
            deltas = [
                librosa.feature.delta(feature_data, order=i, axis=0).astype("float32")
...
def _get_audio_linear_spectrogram(
    audio, sample_rate, window_len=0.025, step_len=0.010, num_feature_filters=512, center=True
):
    """
    ...
    spectrogram = numpy.abs(
        stft_func(
            audio,
            hop_length=int(step_len * sample_rate),
            win_length=int(window_len * sample_rate),
            n_fft=num_feature_filters * 2,
            center=center,
        )
    )
...
def _get_audio_features_mfcc(
    ...
    energy = rms_func(
        y=audio, hop_length=int(step_len * sample_rate), frame_length=int(window_len * sample_rate), center=center
    )
...
def _get_audio_db_mel_filterbank(
    audio,
    sample_rate,
    window_len=0.025,
    step_len=0.010,
...
def _get_random_permuted_audio(audio, sample_rate, opts, random_state):
    ...
    if opts.get("rnd_zoom_switch", 1.0) > 0.0:
    ...
    if opts.get("rnd_pitch_switch", 1.0) > 0.0:
        opts.get("rnd_pitch_lower"), opts.get("rnd_pitch_upper", 1.0)  # Mark as read.
    if random_state.uniform(0.0, 1.0) < opts.get("rnd_pitch_switch", 0.2):
        n_steps = random_state.uniform(opts.get("rnd_pitch_lower", -1.0), opts.get("rnd_pitch_upper", 1.0))
