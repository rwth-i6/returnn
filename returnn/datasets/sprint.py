        ...
        with self.lock:
            ...
            if self.predefined_seq_list_order:
                ...
                assert seq_idx < len(self.predefined_seq_list_order), "seq_idx %i, expected predef num seqs %i" % (
                    seq_idx,
                    len(self.predefined_seq_list_order),
                )
                ...
                if expected_seq_name != segment_name:
                    if segment_name in self.predefined_seq_list_order:
                        raise Exception(
                            "seq_idx %i expected to be tag %r but got tag %r; tag %r is at idx %i"
                            % (
                                seq_idx,
                                expected_seq_name,
                                segment_name,
                    ...
                    raise Exception(
                        "seq_idx %i expected to be tag %r but got tag %r; tag %r not found"
            ...
            if seq_idx > self.requested_load_seq_end - 1 + self.SprintCachedSeqsMax:
                print(
                    "%s add_new_data: seq=%i, len=%i. Cache filled, waiting to get loaded..."
                    % (self, seq_idx, num_frames),
                    file=log.v5,
                )
...
class ExternSprintDataset(SprintDatasetBase):
    ...
    def _start_child(self, epoch, get_dim_only=False):
        ...
        if pid == 0:  # child
            ...
            try:
                ...
                print("%s child exec failed." % self)  # noqa
        ...
        if get_dim_only:
            ...
            self.reader_thread = Thread(
                target=self._reader_thread_proc, args=(pid, epoch), name="%s reader thread" % self
            )
    ...
    def _reader_thread_proc(self, child_pid, epoch):
        ...
        try:
            ...
            print(
                "%s (proc %i) finished reading epoch %i, seen all %r (finished), num seqs %i"
                % (self, child_pid, epoch, have_seen_the_whole, seq_count),
                file=log.v5,
            )
