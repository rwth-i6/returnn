...
def copy_with_new_split_axes(
    ...
    for idxs in numpy.ndindex(tuple([len(parts) for parts in old_axis_splits])):
        ...
        dims = [
            min(old_parts[i], new_parts[i]) for i, old_parts, new_parts in zip(idxs, old_axis_splits, new_axis_splits)
        ]
...
def opt_register_grad_func(op_type, grad_func, assert_is_same=True):
    ...
    if f is not None:
        if assert_is_same:
            assert f is grad_func, "already registered grad for %r, and not the same func: %r != %r" % (
                op_type,
                f,
                grad_func,
            )
        ...
        with tf.control_dependencies(
            ...
            if with_grad:
                ...
                opt_register_grad_func(
                    op_type=grad_name, grad_func=_identity_with_check_numerics_grad, assert_is_same=False
                )
...
def check_shape_equal(x, y):
    ...
    with tf.name_scope("check_shape_equal"):
        with tf.control_dependencies(
            [
                tf_compat.v1.assert_equal(
                    tf.shape(x), tf.shape(y), data=["x.shape not y.shape", tf.shape(x), tf.shape(y)]
                )
            ]
        ):
...
def get_shape(x):
    ...
    with tf.name_scope("get_shape"):
        ...
        return [
            static_shape.dims[i].value if static_shape.dims[i].value is not None else dyn_shape[i]
...
def setup_tf_thread_pools(num_threads=None, log_file=None, tf_session_opts=None):
    ...
    if log_file:
        print(
            "Setup TF inter and intra global thread pools, num_threads %r, session opts %r." % (num_threads, opts),
            file=log_file,
        )
...
def check_initial_tf_thread_pool_init(tf_session_opts: Dict[str, Any] = None):
    ...
    if not _setup_tf_thread_pools_called_once:
        ...
        print(
            "setup_tf_thread_pools() not yet called (via func %s), calling it now."
...
def print_available_devices(tf_session_opts=None, file=None):
    ...
    if tf_session_opts and tf_session_opts.get("gpu_options", {}).get("visible_device_list", None):
        print(
            "TF session gpu_options.visible_device_list is set to %r."
            % (tf_session_opts["gpu_options"]["visible_device_list"],),
            file=file,
        )
    ...
    for dev in devs:
        if dev.device_type == "GPU":
            ...
            print(
                ...
                % (util.get_hostname(), dev_id, dev_name, util.human_bytes_size(memory_limit_bytes)),
                file=file,
            )
...
def dot(a, b, transpose_b=False):
    ...
    with tf.name_scope("dot"):
        ...
        if a_ndim != 2 or b_ndim != 2:
            res_shape = [get_shape_dim(a, i) for i in range(0, a_ndim - 1)] + [
    ...
    with tf.name_scope("swapaxes"):
        ...
        if ndim is not None:
            if isinstance(axis1, tf.Tensor) or isinstance(axis2, tf.Tensor):
                perm = [
                    tf.where(tf.equal(axis1, i), axis2, tf.where(tf.equal(axis2, i), axis1, i)) for i in range(ndim)
                ]
            else:
            ...
            perm = tf.where(
                tf.equal(axis1_bc, all_axes), axis2_bc, tf.where(tf.equal(axis2_bc, all_axes), axis1_bc, all_axes)
            )
...
def tile_transposed(x, axis, multiples):
    ...
    with tf.name_scope("tile_transposed"):
        ...
        y = tf.reshape(
            x, [shape[i] for i in range(axis)] + [shape[axis] * multiples] + [shape[i] for i in range(axis + 1, ndim)]
        )
        cache.set_cache(y)
        return y
    ...
    with tf.name_scope("sparse_labels"):
        ...
        if collapse_repeated:
            with tf.name_scope("collapse_repeated"):
                diffs = tf.concat(
                    axis=1, values=[tf.ones_like(x[:, :1], dtype=tf.bool), tf.not_equal(x[:, 1:], x[:, :-1])]
                )  # shape (batch,time)
...
class OpCodeCompiler(NativeCodeCompiler):
    ...
    def __init__(
        ...
        cuda_auto_min_compute_capability=True,
        include_paths=(),
        ld_flags=(),
        ...
        with reuse_name_scope_of_tensor(gradient):
            ...
            if sparse_grads and isinstance(gradient, tf.IndexedSlices):
                gradient = tf.IndexedSlices(
                    values=gradient_values, indices=gradient.indices, dense_shape=gradient.dense_shape
                )
            else:
...
class MetaLosses:
    ...
    @classmethod
    def tikhonov_regularized(cls, x, dummy, loss_scale=1.0, loss_name=None, loss_source=None):
        """
        :param tf.Tensor x:
...
def nd_indices(indices, batch_axis=0, indices_batch_major=None):
    ...
    with tf.name_scope("nd_indices"):
        ...
        for axis in range(indices.get_shape().ndims):
            if axis == batch_axis:
                continue
            batches_idxs = expand_dims_unbroadcast(
                batches_idxs, axis=axis, dim=tf.shape(indices)[axis], name="batches_idxs_bc"
            )  # (batch, ...)
        batches_idxs.set_shape(indices.get_shape())
        if indices_batch_major:
            idxs_exp = tf.stack(
                [batches_idxs, indices], axis=-1, name="idxs_exp"
            )  # (batch,...,2), where the 2 stands for (batch_idx, index)
        else:
            idxs_exp = tf.stack(
                [indices, batches_idxs], axis=-1, name="idxs_exp"
            )  # (...,2), where the 2 stands for (index, batch_idx)
        return idxs_exp
...
def windowed_nd(
    source, window_size, window_left=None, window_right=None, padding="same", time_axis=1, new_window_axis=2, stride=1
):
    """
...
def slice_nd(x, start, size):
    ...
    with tf.name_scope("slice_nd"):
        ...
        mask = tf.logical_or(tf.greater(window_pos, shape[1] - 1), tf.less(window_pos, 0))  # (n_batch*size,) tf.bool
...
def get_global_train_flag_placeholder():
    ...
    :rtype: tf.Tensor
    """
    return global_tensor(lambda: tf_compat.v1.placeholder(tf.bool, shape=(), name="train_flag"), name="train_flag")
...
def encode_raw(x, axis=-1, seq_lens=None):
    ...
    with tf.name_scope("encode_raw"):
        character_lookup = global_tensor(lambda: tf.constant([chr(i) for i in range(256)]), name="character_lookup")
...
def map_labels(x, label_map, name="map_labels"):
    ...
    if isinstance(x, tf.SparseTensor):
        return tf.SparseTensor(
            indices=x.indices, values=map_labels(x.values, label_map=label_map, name=name), dense_shape=x.dense_shape
        )
    with tf.name_scope(name):
        assert label_map
...
def slice_pad_zeros(x, begin, end, axis=0):
    ...
    with tf.name_scope("slice_pad_zeros"):
        ...
        x = tf.cond(tf.less_equal(right_rem, 0), lambda: x, lambda: pad_zeros_in_axis(x, after=right_rem, axis=axis))
...
def global_queue(name, queue_type, capacity, dtypes, shapes=None, names=None):
    ...
    queue_ref = global_tensor(
        name=name, f=lambda: queue_type(capacity=capacity, dtypes=dtypes, shapes=shapes, names=names).queue_ref
    )
...
def init_variable_if_needed(v):
    ...
    maybe_init = tf.cond(tf_compat.v1.is_variable_initialized(v), lambda: tf.no_op(), make_init, name="maybe_init")
...
def raise_OutOfRangeError():
    ...
    with tf.name_scope("raise_OutOfRangeError"):
        queue = global_queue(
            name="raise_exception/queue", queue_type=tf_compat.v1.FIFOQueue, capacity=1, dtypes=[tf.bool]
        )
...
def zeros_dyn_shape(shape, dtype=tf.float32, name="zeros_dyn_shape"):
    ...
    with tf.name_scope(name):
        ...
        zeros = tf.zeros([d if (d is not None) else dyn_one for d in shape], dtype=dtype)
...
def view_as(x, dtype):
    ...
    (y,) = tf_compat.v1.py_func(py_wrap_numpy_view, [x], [dtype], name="py_wrap_numpy_view")
...
def _register_alternative_minmax_grad():
    ...
    opt_register_grad_func(op_type=grad_name, grad_func=_alternative_minmax_grad, assert_is_same=True)
    return grad_name
...
def smoothing_cross_entropy(
    logits, labels, label_smoothing, gaussian=False, vocab_size=None, logits_are_normalized=False
):
    """
    ...
    with tf.name_scope("smoothing_cross_entropy"):
        ...
        normalizing = -(
            confidence * tf_compat.v1.log(confidence)
            + tf.cast(vocab_size - 1, tf.float32) * low_confidence * tf_compat.v1.log(low_confidence + 1e-20)
        )  # scalar
        ...
        if gaussian:
            ...
            soft_targets = normal_dist.prob(
                expand_multiple_dims(
                    tf.cast(tf.range(vocab_size), tf.float32), axes=[i + 1 for i in range(labels.get_shape().ndims)]
                )
            )  # [vocab_size] + shape(labels)
            soft_targets = move_axis(
                soft_targets, old_axis=0, new_axis=labels.get_shape().ndims
            )  # shape(labels) + [vocab_size]
        else:
        ...
        if logits_are_normalized:
            ...
            xentropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=soft_targets)  # shape(labels)
...
def softmax_cross_entropy_over_size(logits, labels, stable_gradient=True):
    ...
    assert any([dim is n_batch for dim in mask_expand_dims_shape]) and any(
        [dim is enc_time_dim for dim in mask_expand_dims_shape]
    )
        ...
        with tf.control_dependencies(
            [
                tf_compat.v1.assert_less_equal(
                    ...
                    numpy.iinfo(numpy.int32).max / 8.0,
...
def create_random_warp_flow_2d(shape, std=None, scale=10.0, blur_std=2.0):
    """
...
def bleu_score(hypothesis, truth, hyp_seq_lens, truth_seq_lens):
    ...
    res = tf_compat.v1.py_func(
        ...
        name="py_bleu_score",
        stateful=False,
        inp=[hypothesis, truth, hyp_seq_lens, truth_seq_lens],
        Tout=tf.float32,
    )
...
def identity_with_debug_log(x, args, out, name="DebugLogOp"):
    ...
    with tf.name_scope(name):
        (y,) = tf_compat.v1.py_func(py_func, [x] + [args[k] for k in arg_keys], [x.dtype], stateful=True)
        with tf.control_dependencies([y]):
            return tf.identity(x)


def add_check_numerics_ops(
    fetches=None, ignore_ops=None, use_check_numerics=True, debug_print_added_checks=True, name="add_check_numerics_ops"
):
    """
            ...
            for output in op.outputs:
                ...
                with tf.control_dependencies(check_op):
                    ...
                    if use_check_numerics:
                        check_op = [
                            tf_compat.v1.check_numerics(output, message=message, name=op.name + "_check_numerics")
                        ]
                    else:
...
def tensor_array_like(ta, **kwargs):
    ...
    return tf.TensorArray(
        dtype=ta.dtype,
        size=ta.size(),
        dynamic_size=tensor_array_is_dynamic_size(ta),
        clear_after_read=tensor_array_is_clear_after_read(ta),
        infer_shape=ta._infer_shape,
        element_shape=tensor_array_element_shape(ta),
        **kwargs,
    )
...
def beam_search(
    scores, beam_size, keep_beams=False, cheating_gold_targets=None, cheating_src_beam_idx=None, cheating_exclusive=True
):
    """
    ...
    if cheating_gold_targets is not None:
        cheating_gold_targets = tf.clip_by_value(cheating_gold_targets, 0, in_dim - 1)  # safety, for invalid values...
        ...
        if cheating_exclusive:
            ...
            cheating_src_beam_idx_bc = expand_multiple_dims(
                cheating_src_beam_idx, axes=[-1] * (2 - cheating_src_beam_idx.shape.ndims)
            )  # [B|1,1]
...
def filter_ended_scores(x, end_flags, batch_dim=None, dim=None, score_zero=0.0, score_rem=-1.0e30):
    """
    ...
    with tf.name_scope("filter_ended_scores"):
        ...
        x.set_shape(
            tf.TensorShape([batch_dim if isinstance(batch_dim, int) else None, dim if isinstance(dim, int) else None])
        )
        ...
        with same_control_flow_ctx(dim):  # force calculation outside loop if possible
            filter_score = tf.one_hot(0, dim, dtype=tf.float32, on_value=score_zero, off_value=score_rem)  # (dim,)
        ...
        with same_control_flow_ctx([dim, batch_dim]):  # force calculation outside loop if possible
            ...
            filter_score.set_shape(
                tf.TensorShape(
                    [batch_dim if isinstance(batch_dim, int) else None, dim if isinstance(dim, int) else None]
                )
            )
...
def kernels_registered_for_op(op_name):
  ...
  """
    native = TFNativeUtilCompiler(base_name="kernels_registered_for_op", code_version=1, code=code, is_cpp=True)
...
def print_graph_output(fetches, file=None, max_depth=None):
    ...
    def p(op, prefix="", indent=0):
        ...
        if isinstance(op, tf.Tensor):
            postfix = " [%i], shape %s" % (
                op.value_index,
                op.shape.as_list() if op.shape.ndims is not None else "<unknown>",
            )
            op = op.op
        assert isinstance(op, tf.Operation)
...
def find_ops_with_tensor_input(tensors, fetches=None, graph=None):
    ...
    if fetches is not None:
        ...
        all_ops = graph_editor.get_backward_walk_ops(fetches, inclusive=True, control_inputs=True, stop_at_ts=tensors)
    else:
        if graph is None:
...
def string_replace(strings, old, new, count=-1):
    ...
    def str_replace(strings, old, new, count):
        ...
        if isinstance(strings, numpy.ndarray):
            return numpy.array([s.replace(old, new, count) for s in strings.flatten()], dtype=strings.dtype).reshape(
                strings.shape
            )
        else:
    ...
    (res,) = tf_compat.v1.py_func(
        str_replace,
        [tf.cast(strings, tf.string), tf.cast(old, tf.string), tf.cast(new, tf.string), tf.cast(count, tf.int32)],
        [tf.string],
        stateful=False,
        name="string_replace",
    )
...
def get_sparse_tensor_length(x):
    ...
    mask = tf_compat.v1.sparse_to_dense(
        x.indices, output_shape=x.dense_shape, sparse_values=tf.ones_like(x.values, dtype=tf.int64)
    )  # prefix+(max_len,)
...
def get_positional_encoding(num_channels, length=None, position=None, min_timescale=1.0, max_timescale=1.0e4):
    ...
    scale = tf.reshape(
        inv_timescales, [1] * len(position.shape) + [num_timescales]
    )  # Usually (1, D//2) or (1, 1, D//2).
...
def get_linear_alignment_out_to_in_indices(input_lens, output_lens, pad_value=0):
    ...
    factors = tf.maximum(tf.cast(input_lens + 1, tf.float32), 0.0) / tf.maximum(
        tf.cast(output_lens + 1, tf.float32), 1.0
    )  # (B,)
...
def get_rnnt_linear_aligned_output(
    input_lens, targets, target_lens, blank_label_idx, pad_value=0, targets_consume_time=False
):
    """
    ...
    idxs = get_linear_alignment_out_to_in_indices(
        input_lens=tf.maximum(out_lens, target_lens), output_lens=target_lens, pad_value=-1
    )  # (B,targetT)
    ...
    idxs2 = tf.scatter_nd(
        indices=nd_indices(idxs), updates=tf.tile(target_times + 1, [batch_dim, 1]), shape=[batch_dim, out_time_dim + 1]
    )  # (B,outT+1)
...
def compute_sampled_logits(
    ...
    with tf.name_scope(name):
        ...
        true_w = tf.slice(all_w, [0, 0], [tf.shape(labels_flat)[0], -1])
        ...
        if remove_accidental_hits:
            acc_hits = tf.nn.compute_accidental_hits(labels, sampled, num_true=num_true)
            ...
            sampled_logits += tf_compat.v1.sparse_to_dense(
                sparse_indices, sampled_logits_shape, acc_weights, default_value=0.0, validate_indices=False
            )
        ...
        out_targets = tf.concat([tf.ones_like(true_logits) / num_true, tf.zeros_like(sampled_logits)], 1)
