...
def greenwood_function(x, scaling_constant=165.4, constant_of_integration=0.88, slope=2.1):
  ...
  return scaling_constant * (np.power(10.0, slope * x) - constant_of_integration)
...
def inv_greenwood_function(freq, scaling_constant=165.4, constant_of_integration=0.88, slope=2.1):
  ...
  :rtype: float
  """
  return np.log10(freq / scaling_constant + constant_of_integration) / slope
...
class GammatoneFilterbankInitializer(init_ops.Initializer):
  ...
  def __call__(self, shape, dtype=None, partition_info=None):
    ...
    fbank = np.reshape(np.transpose(fbank), shape).astype(dtype.as_numpy_dtype)
  ...
  def center_frequencies(num_channels, freq_max, freq_min):
    ...
    x_linear = np.linspace(x_min, x_max, num_channels)
    freq_center = np.vectorize(greenwood_function)(x_linear)
    return freq_center

  @staticmethod
  def bandwidth_by_center_frequency(freq, l_=24.7, q_=9.264491981582191):
    """
    Get bandwidth (named B in [1]) by center frequency
    The default values are taken from RASR, see https://github.com/rwth-i6/rasr/blob/master/src/Signal/GammaTone.cc
    :param freq:
    :param float l_:
    :param float q_: filter quality factor
    :return: bandwidth
    :rtype: float
    """
    return l_ * (1 / (l_ * q_) * freq + 1.0)

  def gammatone_impulse_response(self, f_center, length, sample_rate, k=1., n=4, phi=0.):
    """
    ...
    :param int sample_rate: sample rate of audio signal in Hz
    :param float k: output gain
    :param int n: order of filter
    :param float phi: phase shift
    ...
    num_samples = int(np.floor(sample_rate * length))
    t = np.linspace(1. / sample_rate, length, num_samples)
    return k * np.power(t, n - 1) * np.exp(-2 * np.pi * bandwidth * t) * np.cos(2 * np.pi * f_center * t + phi)

  @staticmethod
  def normalize_filters(filters):
    """
    Normalize filterbank such that the maximum frequency response is 0 dB
    :param np.array filters: filterbank with shape number_channels x filter_length
    ...
    for filt in range(filters.shape[0]):
      ...
      filters[filt, :] = filters[filt, :] / np.max(np.abs(f_resp))
    return filters
