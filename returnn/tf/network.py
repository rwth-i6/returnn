...
class ExternData(TensorDict):
    ...
    def init_from_dataset(self, dataset, auto_create_placeholders=True):
        ...
        for key in data_keys:
            self.data[key] = Tensor(
                ...
                **_data_kwargs_from_dataset_key(dataset=dataset, key=key),
            )
        self.init_batch_info()
    ...
    def get_all_dimension_tags(self, allow_same_feature_dim=False):
        ...
        tags, _ = Dim.get_all_dimension_tags(
            [data for _, data in self.get_sorted_data_items()], dict(allow_same_feature_dim=allow_same_feature_dim)
        )
        return tags
...
def _data_kwargs_from_dataset_key(dataset, key):
    ...
    shape = [None] + list(dataset.get_data_shape(key))  # type: List[Optional[int]]
...
class TFNetwork:
    ...
    def __init__(
        ...
        parent_layer=None,
        parent_net=None,
        extra_parent_net=None,
        extra_name_prefix=None,
        inside_rec_time_dim=None,
        over_rec_time_dim=None,
        over_rec_time_dim_subs=None,
        control_flow_ctx=None,
        absolute_name_prefix=None,
        name="",
    ):
        """
    ...
    def make_extra_net(self, prefix_name, net_name=None, only_template=False, boundary=False):
        ...
        net, _ = base_net._get_extra_net(
            search_flag=self.search_flag, prefix_name=prefix_name, net_name=net_name, boundary=boundary
        )
        if only_template:
            assert boundary
    ...
    def construct_extra_net(
        self,
        net_dict,
        layer_list,
        search_flag=None,
        dep_layers_in_extra=False,
        ...
        for layer_name in layer_list:
            ...
            created_layers.append(
                extra_net.construct_layer(
                    net_dict=net_dict,
                    name=layer_name,
                    check_existing=check_existing,
                    get_layer=get_layer,
                    add_layer=base_add_layer,
                )
            )
    ...
    def construct_layer(
        ...
        if self._extra_layer_name_prefix_pattern.match(name):
            ...
            if self.extra_parent_net:
                ...
                if extra_net is not self:
                    return self.extra_parent_net.construct_extra_net(
                        net_dict=net_dict,
                        layer_list=[full_name],
                        prefix_name=extra_prefix,
                        check_existing=check_existing,
                        base_get_layer=get_layer,
                        base_add_layer=add_layer,
                    )[0]
                if name in net_dict:
            ...
            if explicit_extra_layer_name in net_dict:
                ...
                return self.extra_parent_net.construct_layer(
                    self.extra_parent_net.layers_desc,
                    name=full_name,
                    get_layer=get_layer,
                    add_layer=add_layer,
                    check_existing=check_existing,
        ...
        while True:
            ...
            if subnet:
                ...
                if not subnet.have_layer(base_name):
                    raise LayerNotFound(
                        "sub-layer %r not found in %r" % (base_name, net), layer_name=full_name, network=self
                    )
            ...
            if not sub_layer:
                raise LayerNotFound(
                    "sub-layer %r not found in %r" % (sub_layer_name, root_layer), layer_name=full_name, network=self
                )
            return sub_layer
        ...
        if self._flat_construction_enabled():
            delayed_exc = _DelayedConstructionException(
                ...
                layer_name=name,
                other_kwargs=dict(
                    net_dict=net_dict, get_layer=get_layer, add_layer=add_layer, check_existing=check_existing
                ),
            )
        ...
        if name_with_prefix in self._construction_stack.layers:
            raise NetworkConstructionDependencyLoopException(
                layer_name=name_with_prefix,
                constructing_layers=self._construction_stack.layers,
                net_dict=net_dict,
                network=self,
            )
    ...
    def _create_layer(self, name, layer_class, **layer_desc):
        ...
        debug_print_layer_output = util.CollectionReadCheckCovered.from_bool_or_dict(
            ...
            if "debug_print_layer_output" in layer_desc
            else self.get_config().bool_or_other("debug_print_layer_output", False)
        )
        ...
        )  # also see debug_add_check_numerics_ops
        ...
        with self.layer_creation_scope(layer_class=layer_class, **layer_desc):
            try:
                ...
                out_print_parts = [
                    "[%s]" % ",".join(output_template.get_batch_axes_short_description()),
            ...
            if layer.output.placeholder is not None and debug_print_layer_output.truth_value:
                layer.output.placeholder = py_print(
                    layer.output.placeholder,
                    [layer.get_absolute_name(), layer.output.placeholder],
            ...
            if (
                layer.output.placeholder is not None
                and debug_add_check_numerics_on_output
                and layer.output.dtype.startswith("float")
                and not layer.allow_inf_in_output
            ):
    ...
    def _flatten_layer_with_losses(self):
        ...
        def _check_push_flattening_to_inputs_for_layer(layer_):
            ...
            for dep_ in deps:
                ...
                if _needs_flattening(dep_):
                    if any(
                        d.dimension is None for d in set(dep_.output.dim_tags).difference(dims)
                    ):  # any other dynamic?
                        return False
        ...
        deps_used_by_end_points = {
            layer: {layer} for layer in end_points
        }  # dep -> set(end points), direct and indirect
        ...
        while layer_queue:
            ...
            if not _check_push_flattening_to_inputs_for_layer(layer):
                ...
                mapped_layers[layer] = _make_layer(
                    FlattenBatchLayer,
                    dict(network=layer.network, sources=[layer], name="%s_flat" % layer.name),
                    map_opts=False,
                )
        ...
        for layer, new_layer in mapped_layers.items():
            ...
            new_out_template = new_out_template.copy_add_dim_by_tag(
                new_out.get_batch_dim_tag(), unbroadcast=True, axis=0
            )
            new_out = new_out.copy_compatible_to(new_out_template, add_dims=False)
            cache = tf_util.get_flatten_with_seq_len_mask_cache_for_data(layer.output)
            cache.set_cache(new_out.placeholder)

    def get_fetches_dict(
        self,
        config=None,
        should_train=None,
        should_eval=None,
        ...
        if config.bool("tf_log_memory_usage", False):
            for dev in tf_util.get_tf_list_local_devices():
                ...
                if not tf_util.is_gpu_available_in_session():
                    continue
                d["mem_usage:%s" % os.path.basename(dev.name.replace("/device:", "/"))] = tf_util.mem_usage_for_dev(
                    dev.name
                )
    ...
    def get_default_target(self):
        ...
        if default_target in targets:
            return default_target
        raise Exception(
            "multiple targets %r and default_target %r not in list. set 'target' in config" % (targets, default_target)
        )
    ...
    def get_layer(self, layer_name):
        ...
        if self._extra_layer_name_prefix_pattern.match(layer_name):
            ...
            if layer_name not in extra_net.layers:
                raise LayerNotFound(
                    "layer %r not found in extra net %r" % (layer_name, extra_net),
                    layer_name=orig_layer_name,
                    network=self,
                )
        ...
        if layer_name.startswith("base:"):
            if not self.parent_net:
                raise LayerNotFound(
                    "cannot get layer %r, no parent net for %r" % (layer_name, self),
                    layer_name=orig_layer_name,
                    network=self,
                )
            return self.parent_net.get_layer(layer_name[len("base:") :])
        ...
        if layer_name not in self.layers:
            raise LayerNotFound(
                "layer %r not found in %r" % (layer_name, self), layer_name=orig_layer_name, network=self
            )
        return self.layers[layer_name]
            ...
            with tf_util.reuse_name_scope("", absolute=True, reuse=getattr(tf_compat.v1, "AUTO_REUSE", None)):
                with tf_util.default_control_flow_ctx(), tf.device("/cpu:0"):
                    net._global_train_step_var = tf_compat.v1.get_variable(
                        ...
                        shape=(),
                        dtype=tf.int64,
                        initializer=tf_compat.v1.zeros_initializer(tf.int64),
                        collections=[tf_compat.v1.GraphKeys.GLOBAL_STEP],
                        trainable=False,
                    )
            return net._global_train_step_var
    ...
    def load_params_from_file(self, filename, session):
        ...
        if must_use_custom_checkpoint_loader:
            loader = CustomCheckpointLoader(
                filename=filename, saveable_params=saveable_params, network=self, ignore_missing=ignore_missing_vars
            )
        ...
        try:
            ...
            try:
                ...
                if loader.missing_non_critical_var_names:
                    print(
                        "Did not found non-critical-to-restore vars:",
                        loader.missing_non_critical_var_names,
                        file=log.v2,
                    )
                ...
                print(
                    "Error, some entry is missing in the checkpoint %r: %s: %s" % (filename, type(exc).__name__, exc),
                    file=log.v1,
                )
    ...
    def get_search_choices(
        self, sources=None, src=None, base_search_choice=None, _layer_to_search_choices=None, debug_stream=None
    ):
        """
        ...
        layers = self._get_all_search_choices(
            sources=sources,
            src=src,
            base_search_choice=base_search_choice,
            _layer_to_search_choices=_layer_to_search_choices,
            _normalized_to_layer=normalized_to_layer,
        )
    ...
    def _get_all_search_choices(
        self, sources=None, src=None, base_search_choice=None, _layer_to_search_choices=None, _normalized_to_layer=None
    ):
        """
        Recursively searches through all sources,
        and if there is a :class:`ChoiceLayer` / any layer with search_choices, returns it.
        ...
        if src is not None:
            ...
            if normalized_src != src:
                assert (
                    _normalized_to_layer.setdefault(normalized_src, src) == src
        ...
        if base_search_choice is not None:
            ...
            if normalized_base != base_search_choice:  # from prev frame or so
                ...
                normalized_choices = self._get_all_search_choices(
                    ...
                    _layer_to_search_choices=_layer_to_search_choices,
                    _normalized_to_layer=_normalized_to_layer,
                )
                ...
                if any([layer.get_normalized_layer() == layer for layer in normalized_choices]):
                    ...
                    assert all([layer in _normalized_to_layer for layer in normalized_choices]), "\n".join(
                        [
                            ...
                            "",
                            "Base: %s" % base_search_choice,
                            "",
                            "Cur (normalized) base: %s" % normalized_base,
                            "",
                            "Prev choices:",
                            pformat(layers),
                            "",
                            "Cur (normalized) choices:",
                            pformat(normalized_choices),
                            "",
                            "Mapping:",
                            pformat(_normalized_to_layer),
                            "",
                        ]
                    )
...
class Subnetwork:
    ...
    def __init__(self, parent_net, name, opts=None):
        ...
        self.net = TFNetwork(
            ...
            rnd_seed=0 if template else None,
        )  # seed 0 is not used when we construct the templates
                ...
                self._net_dict.update(
                    {
                        layer_name: {
                            "class": "copy",
                            "from": ["base:%s" % arg for arg in self._from_arg],
                            "dropout": self._dropout,
                            "dropout_noise_shape": self._dropout_noise_shape,
                        }
                    }
                )
    ...
    def _construct_template_subnet(self, get_parent_layer=None):
        ...
        get_templated_layer = GetLayer(
            subnet, subnetwork=self, parent_get_layer=get_parent_layer, add_layer_func=add_templated_layer
        )
        ...
        try:
            ...
            ss = StringIO()
            print(
                "%s: Exception constructing template network (for deps and data shapes): %s %s"
...
class GetLayer:
    ...
    def copy(self):
        ...
        return GetLayer(
            ...
            add_layer_func=self._add_layer_func,
            parent_get_layer=self._parent_get_layer,
        )
    ...
    def __call__(self, layer_name):
        ...
        return get_layer.network.construct_layer(
            net_dict=get_layer._net_dict, name=name, get_layer=get_layer, add_layer=get_layer._add_layer_func
        )
...
class LossHolder:
    ...
    def _tf_summary(self):
        ...
        if self._loss_value is not None:
            ...
            if self._loss_value.get_shape().ndims == 0:
                ...
                if (
                    self._network.get_config().bool("debug_objective_loss_summaries", False)
                    and self._loss_value_for_objective is not None
                ):
    ...
    def _prepare(self):
        ...
        if loss_value is not None:
            if self._network.get_config().bool("debug_add_check_numerics_on_output", False):
                print(
                    "debug_add_check_numerics_on_output: add for layer loss %r: %r"
    ...
    def copy_new_base(self, name=None, layer=None, network=None, reduce_func=None):
        ...
        return LossHolder(
            ...
            only_on_eval=self._only_on_eval,
        )
...
def help_on_tf_exception(
    session, exception, fetches, feed_dict=None, meta_step_info=None, extern_data=None, file=sys.stdout
):
    """
            ...
            try:
                ...
                try:
                    ...
                    if sub_exc.op is op_copied:
                        ...
                        print(
                            "We tried to fetch the op inputs (%r) but got another exception:" % (list(op.inputs),),
                            file=file,
                        )
...
class CustomCheckpointLoader:
    ...
    def __init__(
        ...
        self.custom_param_importers = (
            [
                ...
                if layer.custom_param_importer
            ]
            if network
            else []
        )
    ...
    class CustomParamImporter:
        ...
        def assign_var(self, var, session):
            ...
            print(
                "Custom param import of layer %r with original params %r." % (self.layer, sorted(values_dict.keys())),
                file=log.v3,
            )
    ...
    def _get_param_name(self, v, assert_load_if_prefix_match=True):
        ...
        if self.load_if_prefix:
            if self.load_if_prefix not in v_name:
                assert not assert_load_if_prefix_match, "var %r not expected with load_if_prefix %r" % (
                    v,
                    self.load_if_prefix,
                )
                return None
    ...
    def get_variable_value_map(self):
        ...
        def make_load_weights_nativelstm_to_basic(new_name, postfix):
            ...
            def load_native_lstm_weights():
                ...
                assert (
                    w_re.ndim == w_ff.ndim == 2
                    and w_re.shape[1] == w_ff.shape[1]
                    and w_re.shape[1] // 4 == w_re.shape[0]
                )
        ...
        for v in missing_var_names:
            ...
            if v.endswith("/rec/W_re"):
                ...
                if (
                    old_name_kernel in obsolete_var_names
                    and old_name_bias in obsolete_var_names
                    and cur_name_w in missing_var_names
                    and cur_name_b in missing_var_names
                ):
                    ...
                    old_name2_kernel in obsolete_var_names
                    and old_name2_bias in obsolete_var_names
                    and cur_name_w in missing_var_names
                    and cur_name_b in missing_var_names
                ):
            ...
            if m:
                ...
                matching_obsolete_var_names = [
                    old_name
                    for old_name in obsolete_var_names
        ...
        if self.ignore_missing or not could_not_find_map_list:
            ...
            print(
                "Could not find mappings for these variables:",
                could_not_find_map_list,
                "var_name_map:",
                var_name_map,
                file=log.v3,
            )
