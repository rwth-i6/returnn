...
class InputContext:
    ...
    def __init__(self, parent, extern_data, config, dataset_name, returnn_dataset, iterator):
        ...
        self.returnn_dataset = (
            returnn_dataset  # TODO this might be unset later (if the dataset lives in a separate proc)
        )
    ...
    def get_returnn_dataset(self, **kwargs):
        ...
        return tf.data.Dataset.from_generator(
            generator=generator, output_types=output_types, output_shapes=output_shapes
        )
...
class DatasetDataProvider(DataProviderBase):
    ...
    def __init__(self, extern_data, config, datasets=None):
        ...
        self.iterator = tf_compat.v1.data.Iterator.from_structure(
        ...
        for key, data in extern_data.data.items():
            ...
            for axis_wo_b, dim in enumerate(data.shape):
                if dim is None:  # dynamic length
                    ...
                    assert isinstance(data.size_placeholder[axis_wo_b], tf.Tensor), "next: %r" % (
                        self.iterator_next_element,
                    )
        ...
        self.current_dataset_complete_frac = 0.0
