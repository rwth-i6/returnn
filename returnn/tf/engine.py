...
class Runner(object):
    ...
    def _get_fetches_dict(self):
        ...
        d_fetches = self.engine.network.get_fetches_dict(
            ...
            should_train=self._should_train,
    ...
    def _finalize(self, num_steps):
        ...
        """
        results = {
            key: self._normalize_loss(value, key, self._inv_norm_accumulated)
    ...
    def _horovod_signal_broadcast(self, have_more_data=True, error=False):
        ...
        have_more_data_placeholder = global_tensor(
            ...
            name="horovod_have_more_data_placeholder",
        )  # 0 or 1
        sum_have_data_t = global_tensor(
            lambda: hvd.allreduce(have_more_data_placeholder, average=False), name="horovod_sum_have_data"
        )  # 0..size
        have_error_placeholder = global_tensor(
            lambda: tf_compat.v1.placeholder(tf.int32, shape=(), name="horovod_have_error_placeholder"),
            name="horovod_have_error_placeholder",
        )  # 0 or 1
        sum_have_error_t = global_tensor(
            lambda: hvd.allreduce(have_error_placeholder, average=False), name="horovod_sum_have_error"
        )  # 0..size
    ...
    def _horovod_sync_params(self, local_step, is_final=False):
        ...
        for var in self.engine.updater.trainable_vars:
            assign_ops.append(
                global_tensor(
                    lambda: assign_avg_var(var), name="horovod_sync_params__var_%s" % var.name[:-2].replace("/", "_")
                ).op
            )
        start_time = time.time()
    ...
    def run(self, report_prefix: str, *, raise_exception: bool = False):
        ...
        try:
            ...
            while self.data_provider.have_more_data(session=sess):
                ...
                if (
                    self._should_train
                    and self.reset_updater_vars_mod_step
                    and step % self.reset_updater_vars_mod_step == 0
                ):
                ...
                if step == 0:
                    if (
                        self.engine.config.bool("check_unsupported_device", False)
                        and self.engine.is_requesting_for_gpu()
                    ):
            ...
            print(
                "%s, finished after %i steps, %s elapsed (%.1f%% computing time)"
                % (report_prefix, step, hms(elapsed), (elapsed_tf_percentage * 100.0)),
                file=log.v3,
            )
            ...
            if not isinstance(exc, CancelTrainingException) and not raise_exception:
                help_on_tf_exception(
                    ...
                    extern_data=self.data_provider.extern_data,
                    file=log.v2,
                )
...
class Engine(EngineBase):
    ...
    def init_train_from_config(self, config=None, train_data=None, dev_data=None, eval_data=None):
        ...
        if (
            ...
            and not config.has("extern_data")
            and (train_data or dev_data or eval_data)
        ):
    ...
    def _maybe_update_config(self, net_desc, epoch):
        ...
        def set_value(key, value):
            ...
            if key == "chunking" and self.train_data:
                ...
                (
                    ...
                    self.train_data.custom_chunking_func,
                ) = Dataset._parse_chunking(value)
        ...
        for key in keys:
            ...
            if key in old_orig_config:
                ...
                assert key in self.config.typed_dict, "config update key %r -> %r expected to be in orig. config" % (
                    key,
                    value,
                )
    ...
    @classmethod
    def create_network(
        cls, config, rnd_seed, train_flag, eval_flag, search_flag, net_dict, extern_data=None, initial_learning_rate=1.0
    ):
        """
        ...
        if train_flag is not False and config.list("search_train_network_layers"):
            network.construct_extra_net(
                net_dict,
                layer_list=config.list("search_train_network_layers"),
                search_flag=True,
    ...
    def init_new_network(self, net_desc=None):
            ...
            if self.config.typed_dict.get("restart_after_num_net_reinit", None) is not None:
                ...
                if self._num_net_reinit >= self.config.typed_dict["restart_after_num_net_reinit"]:
                    print(
                        ...
                        % (self._num_net_reinit, self._num_trained_epochs),
                        file=log.v2,
                    )
                    if self.config.is_true("use_horovod"):
        ...
        if self.config.has("init_new_network_callback"):
            self.config.typed_value("init_new_network_callback")(
                ...
                old_network_params=old_network_params,
            )
    ...
    def train(self):
        ...
        if self.start_epoch > final_epoch:
            print(
                "No epochs to train, start_epoch: %i, final_epoch: %i" % (self.start_epoch, self.final_epoch),
                file=log.v1,
            )
    ...
    def _maybe_use_better_last_model(self):
        ...
        if last_best_epoch and last_best_epoch != self.epoch - 1:
            print(
                "Last epoch %i (score: %f) is not the optimal model"
                ...
                % (
                    last_best_epoch,
                    self.learning_rate_control.get_epoch_error_value(last_best_epoch),
    ...
    def train_epoch(self):
        ...
        if (
            self.network.get_graph_reset_callbacks()
            and
        ...
        print(
            self.get_epoch_str(),
            "score:",
            self.format_score(trainer.score),
            "error:",
            self.format_score(trainer.error),
            "elapsed:",
            hms(trainer.elapsed),
            file=log.v1,
        )
    ...
    def eval_model(
        ...
        if output_per_seq_file:
            ...
            assert loss_name in losses_dict, "Unknown loss defined. Got %r. Possible losses are %r" % (
                loss_name,
                losses_dict.keys(),
            )
        ...
        if output_per_seq_file:
            ...
            if dataset.have_corpus_seq_idx():
                ...
                print(
                    "Dataset have_corpus_seq_idx == True, i.e. it will be sorted for optimal performance.", file=log.v3
                )
                dataset.seq_ordering = "sorted_reverse"
            else:
                print(
                    "Dataset have_corpus_seq_idx == False, i.e. it will not be sorted for optimal performance.",
                    file=log.v3,
                )
            ...
            with open(output_per_seq_file, "w") as f:
                if output_per_seq_file_format == "txt":
                    for seq_idx in range(len(results_per_seq)):
                        ...
                        value_list = [
                            " ".join(map(str, v)) if isinstance(v, numpy.ndarray) else str(v) for v in value_list
                        ]
                    ...
                    for seq_idx in range(len(results_per_seq)):
                        ...
                        with numpy.printoptions(threshold=sys.maxsize):
                            f.write(
                                ", ".join(
                                    [
                                        "%r: %r" % (req_out, results_per_seq[seq_tag][req_out])
                                        for req_out in output_per_seq_format
                                    ]
                                )
                            )
                        f.write("},\n")
    ...
    def cleanup_old_models(self, ask_for_confirmation=False):
        ...
        if not self._do_save():
            return
        super().cleanup_old_models(ask_for_confirmation=ask_for_confirmation)
    ...
    def check_uninitialized_vars(self):
        ...
        with tf.name_scope("check_uninitialized_vars"), self.tf_session.graph.as_default():
            ...
            var_mask = tf.logical_not(tf.stack([tf_compat.v1.is_variable_initialized(v) for v in var_list])).eval(
                session=self.tf_session
            )
            ...
            if uninitialized_vars:
                print(
                    "Note: There are still these uninitialized variables: %s" % [v.name for v in uninitialized_vars],
                    file=log.v3,
                )
    ...
    def search(self, dataset, do_eval=True, output_layer_names="output", output_file=None, output_file_format="txt"):
            ...
            for batch_idx in range(len(seq_idx)):
                ...
                for output_layer_idx in range(num_output_layers):
                    if out_beam_sizes[output_layer_idx] is None:
                        print(
                            "seq_idx: %i, seq_tag: %r, output %r: %r"
                            % (
                                seq_idx[batch_idx],
                                seq_tag[batch_idx],
                                output_layer_names[output_layer_idx],
                                outputs[output_layer_idx][batch_idx],
                            ),
                            file=log.v4,
                        )
                        out_idx = batch_idx
                        ...
                        print(
                            "seq_idx: %i, seq_tag: %r, outputs %r: %r"
                            % (
                                seq_idx[batch_idx],
                                seq_tag[batch_idx],
                                output_layer_names[output_layer_idx],
                                outputs[output_layer_idx][beam_start_idx:beam_end_idx],
                            ),
                            file=log.v4,
                        )
                    ...
                    if serialized_outputs[output_layer_idx]:
                        ...
                        if out_beam_sizes[output_layer_idx] is None:
                            print("  hyp:", serialized_outputs[output_layer_idx][out_idx], file=log.v4)
                        else:
                            assert beam_scores[output_layer_idx] is not None
                            for beam_idx in range(out_beam_sizes[output_layer_idx]):
                                print(
                                    "  hyp %i, score %f:"
                                    % (beam_idx, beam_scores[output_layer_idx][batch_idx][beam_idx]),
        ...
        for output_layer_idx in range(num_output_layers):
            ...
            extra_fetches["beam_scores_" + output_layer_names[output_layer_idx]] = output_layer_beam_scores[
                output_layer_idx
            ]
        ...
        print(
            "Search done. Num steps %i, Final: score %s error %s"
            % (runner.num_steps, self.format_score(runner.score), self.format_score(runner.error)),
            file=log.v1,
        )
        if output_file:
            assert out_cache
    ...
    def search_single(self, dataset, seq_idx, output_layer_name=None):
        ...
        output_d = self.run_single(
            dataset=dataset,
            seq_idx=seq_idx,
            output_dict={"output": output_t, "seq_lens": output_seq_lens_t, "beam_scores": output_layer_beam_scores_t},
        )
    ...
    def search_single_seq(self, sources, output_layer_name=None):
        ...
        dataset = StaticDataset(
            data=[{"data": source_seq, "classes": targets_empty_seq} for source_seq in source_seqs],
    ...
    def compute_priors(self, dataset, config=None):
        ...
        forwarder = Runner(
            ...
            train=False,
            eval=False,
            extra_fetches={"outputs": output_layer.output.get_placeholder_flattened()},
            ...
            def _do_post(self):
                ...
                if input_audio_feature_extractor:
                    try:
                        ...
                        print(
                            "Error reading audio (%s). Invalid format? Size %i, first few bytes %r."
                            % (exc, len(f.getbuffer().tobytes()), f.getbuffer().tobytes()[:20]),
                            file=log.v2,
                        )
                        raise
                    audio_len = float(len(audio)) / sample_rate
                    print(
                        "audio len %i (%.1f secs), sample rate %i" % (len(audio), audio_len, sample_rate), file=log.v4
                    )
