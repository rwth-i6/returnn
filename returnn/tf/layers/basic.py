...
class _ConcatInputLayer(LayerBase):
  ...
  def __init__(self, dropout=0, dropout_noise_shape=None, dropout_on_forward=False, mask=None, **kwargs):
    ...
    :param dict[str|tuple,int|None] dropout_noise_shape: see :func:`tf.util.data.get_bc_shape`
...
class ScaledGradientLayer(CopyLayer):
  ...
  Uses :func:`tf.util.basic.scaled_gradient`, or :func:`tf.stop_gradient`
  """
...
class ActivationLayer(_ConcatInputLayer):
  ...
  See :func:`tf.util.basic.get_activation_function` about supported functions.
...
class ScatterNdLayer(_ConcatInputLayer):
  ...
  def __init__(self, position, position_axis, output_dim_via_time_from, filter_invalid_indices=False, **kwargs):
    ...
    # Now we need to implement a similar logic as `tf.util.basic.nd_indices`, but more generic.
    idxs = [
...
class SeqLenMaskLayer(_ConcatInputLayer):
  ...
  def build_mask(cls, x, axis="T", seq_len_source=None, start=None, window_start=None, window_size=None):
    ...
    :return: mask which is broadcastable to energy_data, thus you can e.g. use :func:`tf.util.basic.where_bc`
    :rtype: tf.Tensor
    """
...
class UnflattenNdLayer(_ConcatInputLayer):
  ...
  This basically wraps :func:`tf.util.basic.unflatten_nd`.
  """
...
class SwapAxesLayer(_ConcatInputLayer):
  """
  Swaps two axes. Basically a wrapper around :func:`tf.util.basic.swapaxes`.
...
class SwitchLayer(LayerBase):
  """
  Wrapper around ``tf.where()`` (or more generically :func:`tf.util.basic.where_bc`),
...
class VariableLayer(LayerBase):
  ...
  def __init__(self, shape, dtype="float32", add_batch_axis=True, add_time_axis=False, trainable=True,
    ...
    :param str|float|int init: see :func:`tf.util.basic.get_initializer`
    """
...
class TikhonovRegularizationLayer(CopyLayer):
  """
  Adds the Tikhonov regularization as a meta-loss (see :class:`tf.util.basic.MetaLosses`).
  """
...
class PrintLayer(LayerBase):
  """
  Prints the sources to console/log, via :func:`tf.util.basic.py_print`.
  """
  layer_class = "print"
...
class CrossEntropyLoss(Loss):
  ...
  def __init__(self,
    ...
    :param float label_smoothing: 0.1 is a common default. see :func:`tf.util.basic.smoothing_cross_entropy`
    :param bool label_smoothing_gaussian: see :func:`tf.util.basic.smoothing_cross_entropy`
    :param bool debug_dump:
    :param dict[str] safe_log_opts: passed to :func:`safe_log`
    :param bool use_fused: if possible, use fused opts
    :param float|None fake_upper_bound: uses :func:`tf.util.basic.minimum_with_identity_grad`.
