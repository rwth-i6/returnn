...
class LayerBase:
    ...
    def _set_prev_state(self, state):
        ...
        if not self._rec_previous_layer:
            ...
            def _map_to_state_tensor_simple(state_layer):
                ...
                assert state_layer.output.batch_ndim <= 2, "%s with state %s expects to operate a single step" % (
                    self,
                    state,
                )
            ...
            self._rec_previous_layer.rec_vars_outputs["state"] = nest.map_structure(_map_to_state_tensor_simple, state)
            return
    ...
    @classmethod
    def _base_get_out_data_from_opts(
        cls,
        network,
        name,
        out_type=None,
        ...
        **kwargs,
        ...
        if in_dim:
            assert len(sources_data_list) == 1, "%r: with specific in_dim %s, there must be a single source" % (
                name,
                in_dim,
            )
        ...
        if all([k not in out_type for k in Data.SpecialAxesNames + ("dim_tags", "dims", "shape")]):
            if sources_data:
                ...
                if (
                    not out_type.get("sparse", False)
                    and not out_type.get("sparse_dim", None)
                    and sources_data.feature_dim_axis_or_unspecified is not NotSpecified
                ):
    ...
    def _post_init_output(
        ...
        **kwargs,
        ...
        if network.eval_flag and size_target:
            sources_data.append(
                cls._static_get_target_value(
                    target=size_target,
                    _target_layers=_target_layers,
                    search_choices=_src_common_search_choices,
                    network=network,
                    mark_data_key_as_used=network.eval_flag,
                )
            )
            ...
            sources_data.append(
                cls._static_get_target_value(
                    ...
                    search_choices=_src_common_search_choices,
                    network=network,
                    mark_data_key_as_used=network.train_flag is not False,
                )
            )
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        if not isinstance(src_names, (list, tuple)):
            src_names = [src_names]
        d["sources"] = [get_layer(src_name) for src_name in src_names if not src_name == "none"]
        ...
        if d.get("initial_output", None):  # see get_rec_initial_output
            ...
            if isinstance(initial_output, str):
                if initial_output not in [
                    "zeros",
                    "ones",
                    "var",
                    "keep_over_epoch",
                    "keep_over_epoch_no_init",
                    "apply(0)",
                ]:
        ...
        if "n_out" not in d and "out_dim" not in d and targets:
            ...
            guessed_out_dim = cls._guess_out_dim_from_target_and_opt_loss(
                ...
                loss_class_name=d.get("loss", None),
                get_layer=get_layer,
            )
            if guessed_out_dim:
            ...
            if param_shape != values.shape:
                if copy_param_mode == "subset":
                    ...
                    if param_axes_split_info:
                        ...
                        print(
                            "Param %r: transform old values of shape parts %r into new shape parts %r."
                            % (param, old_axes_splits, param_axes_split_info),
                            file=log.v3,
                        )
                        values = tf_util.copy_with_new_split_axes(
                            old_axis_splits=old_axes_splits,
                            new_axis_splits=param_axes_split_info,
                            old_values=values,
                            new_values=new_values,
                        )
                    else:
                        print(
                            "Param %r: transform old values of shape %r into new shape %r."
                            % (param, values.shape, param_shape),
                            file=log.v3,
                        )
                        values = tf_util.copy_with_new_split_axes(
                    ...
                    print(
                        "Will not set param %r because its shape %s != %s." % (param, shape.as_list(), values.shape),
                        file=log.v3,
                    )
                    continue
    ...
    def _static_get_target_value(
        target, network, mark_data_key_as_used=True, _target_layers=None, get_layer=None, search_choices=None
    ):
        """
        :param str target:
        :param dict[str,LayerBase]|None _target_layers: if target.startswith("layer:"), then this is target -> layer
    ...
    def _get_target_value(self, target=None, mark_data_key_as_used=True, search_choices=NotSpecified):
        ...
        return self._static_get_target_value(
            target=target,
            _target_layers=self._target_layers,
            search_choices=search_choices,
            network=self.network,
            mark_data_key_as_used=mark_data_key_as_used,
        )
    ...
    def get_losses(cls, name, network, output, loss=None, reduce_func=None, layer=None, **kwargs):
        ...
        return [
            LossHolder(name=name, network=network, loss=loss, layer_output=output, layer=layer, reduce_func=reduce_func)
        ]
    ...
    def batch_norm(
        ...
        with reuse_name_scope(self.get_absolute_name_scope_prefix() + "batch_norm", absolute=True):
            ...
            with self.var_creation_scope():
                sample_variance = self.add_param(
                    tf_compat.v1.get_variable(
                        shape=stats_shape,
            ...
            if use_std:
                with self.var_creation_scope():
                    ...
                    gamma_initializer = get_initializer(
                        gamma_init,
                        seed=self.network.random.randint(2**31) if gamma_init else 0,
            ...
            if use_shift:
                with self.var_creation_scope():
                    ...
                    beta_initializer = get_initializer(
                        beta_init,
                        seed=self.network.random.randint(2**31) if beta_init else 0,
                        eval_local_ns={"layer": self},
                    )
                    beta = self.add_param(
                        tf_compat.v1.get_variable(
            ...
            use_fused = (
                tf_util.tf_version_tuple() >= (2, 0, 0)
                and param_version >= 2  # TF2 required for exponential_avg_factor
                and not masked_time
                and use_shift
                and use_std
                and use_sample == 0
            ...
            def _calc_batch_norm_fused(train_flag):
                ...
                if data.feature_dim_axis == data.batch_ndim - 1:  # feature last
                    ...
                    x = tf.reshape(
                        ...
                        [
                            ...
                            util.prod(x_shape[data.feature_dim_axis + 1 :]),
                            1,
                        ],
                    )
                bn_, sample_mean_, sample_variance_ = tf_compat.v1.nn.fused_batch_norm(
                    ...
                    epsilon=epsilon,
                    exponential_avg_factor=momentum,
            ...
            def _calc_batch_norm(train_flag):
                ...
                if update_sample:
                    updated_sample_mean = tf_compat.v1.assign_add(
                        sample_mean, (mean_cur_batch - sample_mean) * momentum
                    )
                ...
                if force_sample or train_flag:
                    if use_sample == 1:
                        ...
                        mean = (1.0 - use_sample) * mean_cur_batch + use_sample * sample_mean_
                        variance = (1.0 - use_sample) * variance_cur_batch + use_sample * sample_variance_
                else:
                    # use_sample = 1.
                    mean, variance = sample_mean_, sample_variance_
            ...
            if use_fused:
                bn, op = self.network.cond_on_train(
                    lambda: _calc_batch_norm_fused(True), lambda: _calc_batch_norm_fused(False)
                )
            else:
...
class DataNotAvailableLayer(InternalLayer):
    ...
    def get_sub_layer(self, layer_name):
        ...
        return DataNotAvailableLayer(
            name="%s/%s" % (self.name, layer_name),
            network=self.network,
            output=out,
            layer_class=sub_layer_class,
            layer_desc=opts,
        )
...
class ReuseParams:
    ...
    def from_config_dict(cls, opts, network, get_layer):
        ...
        if "map" in opts:  # share specific parameters
            assert isinstance(opts["map"], dict), "reuse_params['map'] should be a dict but is %s" % (
                type(opts["map"]),
            )
        ...
        def create_dummy_layer(self):
            ...
            print(
                (
                    ...
                    + "thus creating it on dummy inputs now"
                )
                % self.layer_name,
                file=log.v4,
            )
    ...
    def __init__(
        self, reuse_layer=None, map=None, custom=None, auto_create_missing=False, layer_output=None, shape=None
    ):
        """
    ...
    def get_variable_scope(self, base_layer, **kwargs):
        ...
        with tf_compat.v1.variable_scope(
            tf_compat.v1.get_variable_scope(), custom_getter=_variable_custom_getter, **kwargs
        ) as scope:
            return scope
...
class SearchChoices:
    ...
    def set_beam_from_rec(self, rev_vars_outputs):
        ...
        """
        assert (
            rev_vars_outputs.get("choice_scores", None) is not None
            and rev_vars_outputs.get("choice_src_beams", None) is not None
        )
...
class Loss:
    ...
    def __init__(
        ...
        scale=1.0,
        _check_output_before_softmax=None,
    ):
        """
        ...
        if custom_inv_norm_factor:
            assert custom_norm_factor is None, (
                "%s: do not provide both custom_norm_factor and custom_inv_norm_factor" % self
            )
        self.scale = scale
        ...
        with tf.name_scope("loss_init"):
            ...
            if output.feature_dim_axis is not None and output.feature_dim_axis != output.batch_ndim - 1:
                if output_with_activation:
                    ...
                    output_with_activation = OutputWithActivation(
                        x=move_axis(output_with_activation.x, output.feature_dim_axis, -1),
            ...
            if self.output.have_time_axis() and self.output.have_batch_axis():
                ...
                if self.output.have_batch_axis():
                    self.loss_norm_factor = 1.0 / tf.cast(
                        tf.shape(self.output.placeholder)[self.output.batch_dim_axis], tf.float32
                    )
                else:
                    self.loss_norm_factor = 1.0
                if target:
    ...
    def _check_init(self):
        ...
        if self.output_before_softmax_flat is not None or self.output_flat is not None:
            assert self.target_flat is not None, "%s: have flat output (%r) but not flat targets (%r)" % (
                self,
                self.output,
                self.target,
            )
                ...
                if self.output.have_batch_axis() and self.target.have_batch_axis():
                    ...
                    checks += [
                        tf.Assert(
                            ...
                            [
                                "Loss_debug_runtime_sanity_checks",
                                "batch dim mismatch",
                                "output:",
                                str(self.output),
                                "shape",
