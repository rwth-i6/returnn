...
class AlternatingRealToComplexLayer(_ConcatInputLayer):
    ...
    def __init__(self, **kwargs):
        ...
        self.output.size_placeholder = {
            0: self.input_data.size_placeholder[self.input_data.time_dim_axis_excluding_batch]
        }
    ...
    def get_out_data_from_opts(cls, name, sources, n_out=None, **kwargs):
        ...
        return super(AlternatingRealToComplexLayer, cls).get_out_data_from_opts(
            name=name,
            sources=sources,
            out_type={"dim": n_out, "dtype": "complex64", "batch_dim_axis": 0, "time_dim_axis": 1},
            **kwargs,
        )
...
class BatchMedianPoolingLayer(_ConcatInputLayer):
    ...
    def __init__(self, pool_size=1, **kwargs):
        ...
        median = tf.nn.top_k(
            reshaped_input, k=tf.cast(tf_compat.v1.ceil(tf.constant(pool_size, dtype=tf.float32) / 2.0), dtype=tf.int32)
        ).values[:, :, :, -1]
        ...
        self.output.size_placeholder = {
            self.output.time_dim_axis_excluding_batch: tf.strided_slice(
                ...
                [0],
                tf.shape(self.input_data.size_placeholder[self.input_data.time_dim_axis_excluding_batch]),
                [pool_size],
            )
        }
        ...
        return Data(
            ...
            size_placeholder={
                0: tf.strided_slice(
                    ...
                    [0],
                    tf.shape(input_data.size_placeholder[input_data.time_dim_axis_excluding_batch]),
                    [pool_size],
                )
            },
            sparse=False,
...
class ComplexLinearProjectionLayer(_ConcatInputLayer):
    ...
    def _build_clp_multiplication(self, clp_kernel):
        ...
        output_real = tf.einsum("btf,fp->btp", input_real, kernel_real) - tf.einsum(
            "btf,fp->btp", input_imag, kernel_imag
        )
        output_imag = tf.einsum("btf,fp->btp", input_imag, kernel_real) + tf.einsum(
...
class MaskBasedGevBeamformingLayer(LayerBase):
    ...
    def __init__(self, nr_of_channels=1, postfilter_id=0, qralgorithm_steps=None, output_nan_filter=False, **kwargs):
        ...
        complex_spectrogram = tf.transpose(
            tf.reshape(
                complex_spectrogram,
                (
                    tf.shape(complex_spectrogram)[0],
                    tf.shape(complex_spectrogram)[1],
                    nr_of_channels,
                    tf.shape(complex_spectrogram)[2] // nr_of_channels,
                ),
            ),
            [0, 1, 3, 2],
        )
        masks = tf.transpose(
            ...
            [
                self.sources[1].output.batch_dim_axis,
                self.sources[1].output.time_dim_axis,
                self.sources[1].output.feature_dim_axis,
            ],
        )
        masks = tf.transpose(
            tf.reshape(
                masks, (tf.shape(masks)[0], tf.shape(masks)[1], nr_of_channels, tf.shape(masks)[2] // nr_of_channels)
            ),
            [0, 1, 3, 2],
        )
        ...
        gev_bf = TfMaskBasedGevBeamformer(
            flag_inputHasBatch=1,
            tfFreqDomInput=complex_spectrogram,
            tfNoiseMask=noise_masks,
            tfSpeechMask=speech_masks,
            postFilterId=postfilter_id,
            qrAlgorithmSteps=qralgorithm_steps,
...
class MaskBasedMvdrBeamformingWithDiagLoadingLayer(LayerBase):
    ...
    def __init__(
        self, nr_of_channels=1, diag_loading_coeff=0, qralgorithm_steps=None, output_nan_filter=False, **kwargs
    ):
        """
        :param int nr_of_channels: number of input channels to beamforming (needed to split the feature vector)
        ...
        complex_spectrogram = tf.transpose(
            tf.reshape(
                complex_spectrogram_with_concat_channels,
                (
                    tf.shape(complex_spectrogram_with_concat_channels)[0],
                    tf.shape(complex_spectrogram_with_concat_channels)[1],
                    nr_of_channels,
                    tf.shape(complex_spectrogram_with_concat_channels)[2] // nr_of_channels,
                ),
            ),
            [0, 1, 3, 2],
        )
        noise_masks = self.sources[1].output.get_placeholder_as_batch_major()
        noise_masks = tf.transpose(
            tf.reshape(
                noise_masks,
                (
                    tf.shape(noise_masks)[0],
                    tf.shape(noise_masks)[1],
                    nr_of_channels,
                    tf.shape(noise_masks)[2] // nr_of_channels,
                ),
            ),
            [0, 1, 3, 2],
        )
        ...
        mvdr_bf = TfMaskBasedMvdrBeamformer(
            ...
            tfFreqDomInput=complex_spectrogram,
            tfNoiseMask=noise_masks,
            tfDiagLoadingCoeff=tf.constant(diag_loading_coeff, dtype=tf.float32),
            qrAlgorithmSteps=qralgorithm_steps,
            outputNanFilter=output_nan_filter,
        )
    ...
    def get_out_data_from_opts(cls, out_type=None, n_out=None, **kwargs):
        ...
        out_type["batch_dim_axis"] = 0
        out_type["time_dim_axis"] = 1
        return super(MaskBasedMvdrBeamformingWithDiagLoadingLayer, cls).get_out_data_from_opts(
            out_type=out_type, **kwargs
        )
...
class MultiChannelMultiResolutionStftLayer(_ConcatInputLayer):
        ...
        **kwargs,
        ...
        def _compute_size_placeholder():
            ...
            if self._pad_last_frame:
                new_size = (
                    self.input_data.size_placeholder[0] - self._reference_frame_size - 1
                ) // self._frame_shift + 2
            else:
            ...
            Dim(
                ...
                description="%s:MultiChannelMultiResolutionStft" % self.name,
                dyn_size=new_size,
        ...
        for fft_size, frame_size in zip(self._fft_sizes, self._frame_sizes):
            ...
            if self._use_rfft:
                ...
                if channel_wise_stft_res_list:
                    channel_concatenated_stft = _crop_stft_output_to_reference_frame_size_length(
                        channel_concatenated_stft, get_shape(channel_wise_stft_res_list[0])[1]
                    )
    ...
    def get_out_data_from_opts(cls, fft_sizes, use_rfft=True, nr_of_channels=1, **kwargs):
        ...
        if "n_out" not in kwargs:
            kwargs["n_out"] = n_out
        return (
            super(MultiChannelMultiResolutionStftLayer, cls)
...
class MultiChannelStftLayer(MultiChannelMultiResolutionStftLayer):
    ...
    def get_out_data_from_opts(cls, fft_size, use_rfft=True, nr_of_channels=1, **kwargs):
        ...
        """
        return (
            super(MultiChannelStftLayer, cls)
...
class ParametricWienerFilterLayer(LayerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # noinspection PyShadowingNames
        def _get_parameters_from_constructor_inputs(
            parameters, l_overwrite, p_overwrite, q_overwrite, average_parameters
        ):
        ...
        l, p, q = _get_parameters_from_constructor_inputs(
            parameters, l_overwrite, p_overwrite, q_overwrite, average_parameters
        )
...
class SplitConcatMultiChannel(_ConcatInputLayer):
    ...
    def __init__(self, nr_of_channels=1, **kwargs):
        ...
        output = tf.reshape(
            input_placeholder,
            [
                tf.shape(input_placeholder)[0],
                tf.shape(input_placeholder)[1],
                nr_of_channels,
                tf.shape(input_placeholder)[2] // nr_of_channels,
            ],
        )  # shape B x T x C x (F / C)
        ...
        size_placeholder_np_repeat = tf.reshape(
            self.input_data.size_placeholder[self.input_data.time_dim_axis_excluding_batch], [-1, 1]
        )  # shape T x 1
