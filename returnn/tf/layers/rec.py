...
class RecLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if max_seq_len_via:
            ...
            assert max_seq_len_via.output.batch_shape == (), "%s: max_seq_len_via %s not scalar" % (
                self,
                max_seq_len_via,
            )
        ...
        if cheating:
            print(
                "Warning: cheating is an unused parameter in RecLayer, "
                "to enable cheating set the flag in a ChoiceLayer instead.",
                file=log.v2,
            )
        self._unroll = unroll
            ...
            with self.var_creation_scope():
                ...
                if isinstance(self.cell, _SubnetworkRecCell):
                    ...
                    if self.cell.input_layers_net:
                        self._add_params(
                            params=self.cell.input_layers_net.get_params_list(), scope_name_prefix=scope_name_prefix
                        )
                    ...
                    if self.cell.output_layers_net:
                        self._add_params(
                            params=self.cell.output_layers_net.get_params_list(), scope_name_prefix=scope_name_prefix
                        )
                        self.saveable_param_replace.update(
                            self.cell.output_layers_net.get_saveable_param_replace_dict()
                        )
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        source_data, time_dim_tag = cls.transform_source_and_axis(
            ...
            have_dyn_seq_len_end=have_dyn_seq_len_end,
            opts=d,
            axis=time_dim_tag,
        )
        d["axis"] = time_dim_tag
        ...
        if isinstance(d.get("unit"), dict):
            ...
            subnet = cls.SubnetworkRecCell(
                parent_net=network,
                parent_get_layer=get_layer,
                source_data=source_data,
                time_dim_tag=time_dim_tag,
                net_dict=sub_net_dict,
                rec_layer_name=d["_name"],
            )
            d["unit"] = subnet
        ...
        if not is_gpu_available_in_session():
            ...
            if name.lower() in m:
                if name.lower() not in cls._warn_msg_once_for_cell_name:
                    print(
                        "You have selected unit %r in a rec layer which is for GPU only, so we are using %r instead."
                        % (name, m[name.lower()]),
                        file=log.v2,
                    )
    ...
    def get_losses(cls, name, network, output, loss=None, reduce_func=None, layer=None, **kwargs):
        ...
        if isinstance(unit, _SubnetworkRecCell):  # subnet
            ...
            for layer_name, template_layer in sorted(subnet.layer_data_templates.items()):
                ...
                for loss in template_layer.layer_class_type.get_losses(
                    reduce_func=reduce_func, **template_layer.kwargs
                ):
                    ...
                    loss = loss.copy_new_base(
                        network=network, name="%s/%s" % (name, loss.name), reduce_func=reduce_func
                    )
                    losses.append(loss)
        return losses
    ...
    def _get_cell(self, unit, unit_opts=None):
        ...
        if issubclass(rnn_cell_class, tf_native_op.RecSeqCellOp):
            ...
            cell = rnn_cell_class(
                n_hidden=n_hidden,
                n_input_dim=self.input_data.dim,
    ...
    def _get_output_cell(self, cell):
        ...
        if isinstance(cell, BaseRNNCell):
            with tf_compat.v1.variable_scope(
                tf_compat.v1.get_variable_scope(), initializer=self._fwd_weights_initializer
            ):
                ...
                with tf.control_dependencies(
                    [
                        tf_compat.v1.assert_greater_equal(
                            self._max_seq_len, original_len, message="required for unroll: max_seq_len >= seq_len"
                        )
                    ]
                ):
                    pad_len = tf.maximum(
                        0, self._max_seq_len - original_len
                    )  # max, in case we want to support truncate later
    ...
    @staticmethod
    def _get_cudnn_param_size(
        num_units, input_size, num_layers=1, rnn_mode="lstm", input_mode="linear_input", direction="unidirectional"
    ):
        """
            ...
            (
                numpy.concatenate(
                    [
                        ...
                        numpy.reshape(values[i + 4], [output_dim, output_dim]),
                    ],  # recurrent weights
                    axis=1,
                ),
                values[8 + i] + values[8 + i + 4],  # input bias  # recurrent bias
            )
            for i in range(4)
        ]
        ...
        if self._input_projection:
            if cell.does_input_projection:
                ...
                weights = tf_compat.v1.get_variable(
                    name="W",
                    shape=(self.input_data.dim, cell.n_input_dim),
                    dtype=tf.float32,
...
class _SubnetworkRecCell(object):
    ...
    def __init__(self, net_dict, source_data, time_dim_tag, rec_layer_name, parent_net, parent_get_layer):
        ...
        for key, data in parent_net.extern_data.data.items():
            ...
            self.net.extern_data.data[key] = data.copy_template_excluding_time_dim().copy_template_set_ctx(
                control_flow_ctx
            )
    ...
    def _construct_template(self, parent_get_layer):
        ...
        skip_stack_trace_exception_types = (NetworkConstructionDependencyLoopException,)
                ...
                if exc_key not in cls.collected_exceptions:
                    if isinstance(value, skip_stack_trace_exception_types):
                        ...
                        cls.collected_exceptions[exc_key] = CollectedException(
                            ...
                            % (
                                color(
                                    "EXCEPTION while constructing layer %r" % layer_name, color.fg_colors[1], bold=True
                                ),
        ...
        class RecSubnetGetTemplateLayer:
            ...
            def add_templated_layer(lself, name, layer_class, **layer_desc):
                ...
                assert name in self.layer_data_templates, "%s: Unexpected layer name %r. Templates:\n%s" % (
                    self,
                    name,
                    pformat(self.layer_data_templates),
                )
                ...
                try:
                    ...
                    if lself.got_uninitialized_deps_count > 0:
                        ...
                        raise
                if lself.got_uninitialized_deps_count == 0:
                    layer_.kwargs["output"].sanity_check(
                        ignore_placeholder=True
                    )  # placeholder might be overwritten later
                ...
                try:
                    ...
                    try:
                        res_layer = self.net.construct_layer(
                            ...
                            name=name,
                            get_layer=default_get_layer,
                            add_layer=default_get_layer.add_templated_layer,
                        )
                    ...
                    if not layer_.is_initialized and lself.iterative_testing:
                        ...
                        try:
                            self.net.construct_layer(
                                net_dict=self.net_dict,
                                name=name,
                                get_layer=shallow_get_layer,
                                add_layer=shallow_get_layer.add_templated_layer,
                            )
                        except Exception:
                            ConstructCtx.collect_exception(layer_name=name)
                    ...
                    if initial_output_layer and layer_ in ConstructCtx.partially_finished:
                        ...
                        last_get_layer.add_templated_layer(layer_class=layer_.layer_class_type, **layer_.kwargs)
                    ...
                    if res_layer and res_layer is not layer_:
                        ...
                        lself.add_templated_layer(layer_class=res_layer.layer_class_type, **res_layer.kwargs)
                    ...
                    print(
                        ...
                        % (
                            ...
                            pformat(
                                [
                                    layer
                                    for _, layer in sorted(self.layer_data_templates.items())
    ...
    def _construct(self, prev_outputs, prev_extra, i, data=None, inputs_moved_out_tas=None, needed_outputs=("output",)):
        ...
        for name in set(list(prev_outputs.keys()) + list(prev_extra.keys())):
            if "prev:%s" % name in self.net.layers:
                continue
            try:
                self.net.layers["prev:%s" % name] = prev_layers[name] = self.layer_data_templates[
                    name
                ].copy_as_prev_time_frame(
                    prev_output=prev_outputs.get(name, None), rec_vars_prev_outputs=prev_extra.get(name, None)
                )
            ...
            try:
                ...
                if name == "end":
                    ...
                    with tf.name_scope("end_flag"):
                        layer.output.placeholder = tf.logical_or(
                            prev_end_layer.output.placeholder, layer.output.placeholder
                        )
                return layer
            except Exception as exc:
    ...
    def get_layer_from_outside(self, layer_name):
        ...
        if self.output_layers_net and layer_name in self.output_layers_net.layers:
         ...
         Add 'is_output_layer':True to the layer options."
                % (self.parent_rec_layer, layer_name)
            )
        return None
    ...
    def get_init_loop_vars_shape_invariants(self):
        ...
        init_out_shapes = {
            k: tf.TensorShape(self.layer_data_templates[k].output.batch_shape) for k in self._initial_outputs
        }
    ...
    class OutputToAccumulate:
        ...
        def _make_padded_tensor_array(self, ta):
            ...
            _, max_shape, buffer_ta = tf.while_loop(
                cond=lambda i, *_args: tf.less(i, size), body=_body_infer_max_shape, loop_vars=(0, max_shape, buffer_ta)
            )
            ...
            _, new_ta_padded = tf.while_loop(
                cond=lambda i, *_args: tf.less(i, size), body=_body_pad_max_shape, loop_vars=(0, new_ta_padded)
            )
    ...
    def get_output(self):
        ...
        with tf.name_scope("subnet_base"):
            ...
            if rec_layer.input_data:
                ...
                if rec_layer.target and rec_layer.network.eval_flag:
                    # noinspection PyProtectedMember
                    target_data = rec_layer._get_target_value(target=rec_layer.target, mark_data_key_as_used=False)
            ...
            if fixed_seq_len is not None:
                ...
                assert time_dim_tag == self.time_dim_tag, "%s: fixed_seq_len %s without dim tag?" % (
                    self,
                    fixed_seq_len,
                )
                if fixed_seq_len.get_shape().ndims > 0:
                    ...
                    with tf.name_scope("check_seq_len_batch_size"):
                        ...
                        if time_dim_tag:
                            time_dim_tag.set_tag_on_size_tensor(
                                fixed_seq_len, batch=time_dim_tag.batch, same_as_before=True
                            )
                    ...
                    if common_data_len is None:
                            ...
                            with tf.control_dependencies(
                                [
                                    tf_compat.v1.assert_equal(
                                        tf.reduce_max(input_seq_len),
                                        data_len,
                                        [
                                            "RecLayer %r with sources %r:" % (rec_layer.name, rec_layer.sources),
                                            " The length of the sources (",
                                            tf.reduce_max(input_seq_len),
                                            ") differ from the length of the target ",
                                            key,
                                            "(",
                                            data_len,
                                            ").",
                        ...
                        if fixed_seq_len is not None:
                            with tf.control_dependencies(
                                [
                                    tf_compat.v1.assert_equal(
                                        tf.reduce_max(fixed_seq_len),
                                        data_len,
                                        [
                                            "RecLayer %r:" % (rec_layer.get_absolute_name(),),
                                            " The predefined length (",
                                            tf.reduce_max(fixed_seq_len),
                                            ") differs from the length of the target ",
                                            key,
                                            "(",
                                            data_len,
                                            ").",
                        ...
                        with tf.control_dependencies(
                            [
                                tf_compat.v1.assert_equal(
                                    ...
                                    data_len,
                                    [
                                        "RecLayer %r:" % rec_layer.name,
                                        " The length of all targets (%s) " % ", ".join(used_keys),
                                        " has to be the same. Found length ",
                                        data_len,
                                        " for %s, which does not match length " % key,
                                        common_data_len,
                                        " of the other data.",
                                    ],
                                )
            ...
            def add_output_to_acc(layer_name):
                ...
                outputs_to_accumulate.append(
                    _SubnetworkRecCell.OutputToAccumulate(
                        name=name_, data=template_layer.output, get=lambda: self.net.get_layer(layer_name).output
                    )
                )
            ...
            if layer_names_with_losses:
                ...
                def make_get_loss_in_loop_frame(loss, layer_name, return_error=False, return_loss=False):
                    ...
                    def get_loop_loss():
                        ...
                        assert isinstance(value, tf.Tensor), "layer %r loss %r %s invalid" % (
                            layer,
                            loss,
                            "loss_value" if return_loss else "error_value",
                        )
                ...
                for layer_name in layer_names_with_losses:
                    ...
                    for loss in layer.layer_class_type.get_losses(reduce_func=identity, **layer.kwargs):
                        assert loss.name not in accumulated_loop_losses, "layer %r loss name %r not unique" % (
                            layer,
                            loss.name,
                        )
                ...
                with tf.name_scope("input_layers_moved_out"):
                    ...
                    for layer_name in self.input_layers_moved_out:
                        ...
                        with tf.control_dependencies(
                            [
                                tf.Assert(
                                    tf.greater_equal(layer.output.time_dimension(), max_seq_len),
                                    [
                                        "input TA unstack",
                                        str(layer.output),
                                        "shape",
                                        ...
                                        "seq len",
                                        layer.output.get_sequence_lengths(),
                                        "do not match",
                                        "max seq len",
                                        max_seq_len,
                                    ],
                            ...
                            inp_ta = inp_ta.unstack(
                                layer.output.get_placeholder_as_time_major(), name="%s_ta_unstack" % layer_name
                            )
        ...
        def body(i, net_vars, acc_tas, seq_len_info=None):
            ...
            with reuse_name_scope(rec_layer._rec_scope), reuse_name_scope("while_loop_body"):
                ...
                data_ = {key_: ta.read(i, name="{}_ta_read".format(key_)) for key_, ta in data_tensor_arrays.items()}
                # noinspection PyProtectedMember
                with reuse_name_scope(self.parent_rec_layer._rec_scope):
                    self._construct(
                ...
                def maybe_transform(layer):
                    ...
                    transformed_layer = cur_frame_search_choices_layer.search_choices.translate_to_this_search_beam(
                        layer
                    )
                ...
                for k in sorted(self._initial_outputs):
                    ...
                    if seq_len_info:
                        ...
                        end_flag_data.placeholder = tf.greater_equal(
                            ...
                            i,
                            (fixed_seq_len - 1) if rec_layer.include_eos else fixed_seq_len,
                        )
                    ...
                    layer.output.placeholder = where_bc(
                        condition=rel_end_layer.output.copy_compatible_to(
                            layer.output, check_sparse=False, check_dtype=False
                        ).placeholder,
                        x=prev_layer.output.placeholder,
                        y=layer.output.placeholder,
                    )
                ...
                outputs_flat = [
                    maybe_transform(self.net.layers[k])
                    .output.copy_compatible_to(self.layer_data_templates[k].output)
                    .placeholder
                    for k in sorted(self._initial_outputs)
                ]
                extra_flat = []
                ...
                acc_tas = [
                    out.write_to_tensor_array(acc_ta, index=i) for (acc_ta, out) in zip(acc_tas, outputs_to_accumulate)
                ]
                ...
                if self._debug_out is not None:
                    ...
                    for k in self._initial_extra_outputs:
                        args.update(
                            {"%s.extra.%s" % (k, k2): v for (k2, v) in self.net.layers[k].rec_vars_outputs.items()}
                        )
        ...
        def cond(i, net_vars, acc_tas, seq_len_info=None, allow_inf_max_len=False):
            ...
            with tf.name_scope("loop_cond"):
                ...
                # This is the old behavior. Maybe this might change at some point.
                    ...
                    # noinspection PyProtectedMember
                    assert rec_layer._max_seq_len is None, "%r: unsupported max_seq_len %r" % (
                        rec_layer,
                        rec_layer._max_seq_len,
                    )
        ...
        init_loop_vars = (tf.constant(0, name="initial_i"), self.get_init_loop_vars(), init_acc_tas)
        shape_invariants = (
            tf.TensorShape(()),
        ...
        if self.layers_in_loop:
            ...
            }  # type: typing.Dict[str,typing.Optional[tf.TensorArray]]  # nopep8
        ...
        self._construct_output_layers_moved_out(
            loop_accumulated=self.final_acc_tas_dict,
            seq_len=seq_len,
            extra_output_layers=extra_output_layers,
            final_net_vars=final_net_vars,
        )
                ...
                if accumulated_loop_losses:
                    ...
                    for _, loss in sorted(accumulated_loop_losses.items()):
                        ...
                        loss_value = tensor_array_stack(
                            self.final_acc_tas_dict["loss_%s" % loss.name],
                            stop=max_seq_len,
                        ...
                        if self.final_acc_tas_dict["error_%s" % loss.name] is not None:
                            error_value = tensor_array_stack(
                                self.final_acc_tas_dict["error_%s" % loss.name],
                                stop=max_seq_len,
            ...
            if output_layer:
                ...
                if not self.time_dim_tag.is_dim_known():
                    ...
                    BehaviorVersion.require(
                        ...
                        % (rec_layer, self.time_dim_tag, output_data, output_data.get_time_dim_tag()),
                        version=13,
                    )
                ...
                for tag1, tag2 in zip(rec_layer.output.dim_tags, output_data.dim_tags):
                    try:
                        assert tag1.is_equal(tag2, allow_same_feature_dim=True), "%s vs %s from %s" % (
                            rec_layer.output,
                            output_data,
                            output_layer,
                        )
                ...
                return tensor_array_stack(
                    self.final_acc_tas_dict["output_output"], stop=max_seq_len, name="output_stack"
                )  # e.g. (time, batch, dim)
    ...
    def _opt_search_resolve(self, layer_name, acc_ta, final_net_vars, seq_len, search_choices_cache):
        ...
        latest_batch = latest_layer_choice.output.batch or self.parent_rec_layer.output.batch.copy_set_beam(
            latest_layer_choice.output.beam
        )
        ...
        initial_beam_choices = expand_dims_unbroadcast(initial_beam_choices, axis=0, dim=batch_dim)  # (batch, beam_out)
        ...
        if time_dim_tag.dyn_size_ext and time_dim_tag.dyn_size_ext.placeholder is not None:
            ...
            assert getattr(seq_len, "_RETURNN_dyn_size_beam", NotSpecified) in (
                NotSpecified,
                end_layer_choice.output.batch.beam,
            )
        ...
        new_acc_output_ta = tf.TensorArray(
            name="search_resolved_%s"
            % os.path.basename((acc_ta.handle if acc_ta.handle is not None else acc_ta.flow).op.name),
        ...
        def transform(i, idxs_exp, new_acc_output_ta_):
            ...
            with tf.name_scope("transform_output_%s" % get_valid_scope_name_from_str(layer_name)):
                ...
                output_ = tf.reshape(
                    output_, [batch_dim, layer_choice.output.beam.beam_size] + out_shape[1:], name="split_batch_beam"
                )  # (batch, beam, [n_out])
                output_ = tf.gather_nd(output_, idxs_exp)  # (batch, beam_par, [n_out])
                beam_par = get_shape_dim(idxs_exp, 1)
                output_ = tf.reshape(
                    output_, [batch_dim * beam_par] + out_shape[1:], name="merge_batch_beam"
                )  # (batch * beam_par, [n_out])
        ...
        def search_resolve_body(i, choice_beams, new_acc_output_ta_):
            ...
            with reuse_name_scope(
                (rec_layer._rec_scope.name or "rec") + "/while_loop_search_resolve_body", absolute=True
            ):
                ...
                for choice_ in choice_seq_in_frame:
                    ...
                    if new_acc_output_ta_ is not None and choice_ == layer_choice:
                        new_acc_output_ta_ = transform((i + 1) if is_prev_choice else i, idxs_exp, new_acc_output_ta_)
        ...
        if latest_layer_choice.name not in search_choices_cache:
            ...
            final_choice_rec_vars = self.get_layer_rec_var_from_loop_vars(
                loop_vars=final_net_vars, layer_name=latest_layer_choice.name, final_frame=True, seq_len=seq_len
            )
    ...
    def _move_outside_loop(self, needed_outputs):
        ...
        log_stream = log.v3
        print(
            "Rec layer %r (search %s, train %s) sub net:"
            % (
                self.parent_rec_layer.get_absolute_name(),
                self.net.search_flag,
    ...
    def _construct_output_layers_moved_out(self, loop_accumulated, seq_len, extra_output_layers, final_net_vars):
        ...
        def get_loop_acc_layer(name):
            ...
            with tf.name_scope(in_loop_layer.tf_scope_name):
                ...
                if not time_dim_tag:
                    ...
                    time_dim_tag = self.time_dim_tag.get_for_batch_ctx(
                        batch=batch, ctx=self.parent_net.get_control_flow_ctx()
                    )
                    time_dim_tag.set_tag_on_size_tensor(resolved_seq_len, batch=batch)
                else:
                    self.time_dim_tag.declare_same_as(time_dim_tag)
                output = (
                    in_loop_layer.output.copy_template()
        ...
        def get_layer(name):
            ...
            if name in self.output_layers_moved_out or name.startswith("data:") or name == "data":
                ...
                try:
                    ...
                    self._handle_construct_exception(
                        description="output-net construction of layer %r" % name, exception=exc
                    )
                    raise
            if name not in self.layer_data_templates:
                raise LayerNotFound(
                    ...
                    layer_name=name,
                    network=self.output_layers_net,
                )
        ...
        for name, search_choices in search_choices_cache.items():
            if name not in self.output_layers_net.layers:
                # Create dummy layer.
                output = (
                    self.layer_data_templates[name]
                    .output.copy_template_adding_time_dim(time_dim_axis=0)
...
class _TemplateLayer(LayerBase):
    ...
    def __init__(self, network, name, construct_stack=None, cell=None):
        ...
        dummy_data = Data(
            name="%s:dummy_initial_template_data" % name, dim_tags=[], control_flow_ctx=network.get_control_flow_ctx()
        )
    ...
    def __repr__(self):
        if self.is_initialized:
            return "<%s(%s)(%s) %s%r out_type=%s (construction stack %r)>" % (
                ...
                self.layer_class_type.__name__ if self.layer_class_type else None,
                self.layer_class,
                self.network.get_absolute_name_prefix(),
                self.name,
                self.output.get_description(with_name=False),
    ...
    def get_sub_layer(self, layer_name):
        ...
        opts = opts.copy()
        if "network" in opts:
            assert opts.get("network") and opts.get("name"), (
                "if network is specified, network, name must be set (%r)" % opts
            )
        else:
    ...
    def copy_as_prev_time_frame(self, prev_output=None, rec_vars_prev_outputs=None):
        ...
        if layer.output.placeholder is not None and self.network.get_config().bool(
            "debug_runtime_sanity_checks", False
        ):
...
class _SubnetworkRecWrappedLoss(Loss):
    ...
    def __init__(self, base_loss, loss_value, error_value, norm_factor, seq_lens):
        ...
        super(_SubnetworkRecWrappedLoss, self).__init__(
            ...
            use_flatten_frames=base_loss.use_flatten_frames,
            use_normalized_loss=base_loss.use_normalized_loss,
...
class RnnCellLayer(_ConcatInputLayer):
    ...
    def __init__(
        self, n_out, unit, unit_opts=None, initial_state=None, initial_output=None, weights_init="xavier", **kwargs
    ):
        """
        ...
        **kwargs,
        ...
        with tf.name_scope("rec_initial_state"):
            ...
            if is_namedtuple(type(dim)):
                ...
                if (
                    not isinstance(init_value, dict)
                    and init_value not in (0, 1, None)
                    and not isinstance(init_value, str)
                ):
                    print(
                        (
                            "Layer %r: It is recommended to use a dict to specify 'initial_state'"
                            "with keys %r for the state dimensions %r."
                        )
                        % (name, keys, dim),
                        file=log.v2,
                    )
                if isinstance(init_value, dict):
                    assert set(init_value.keys()) == set(keys), (
                        "You must specify all keys for the state dimensions %r." % dim
                    )
                    assert len(init_value) == len(dim)
                    s = {
    ...
    def get_rec_initial_state_inner(
        ...
        def _get_last_state() -> tf.Tensor:
            ...
            if isinstance(rec_layer, RecLayer) and isinstance(rec_layer.cell, _SubnetworkRecCell):
                ...
                assert (key and state_key == "state") or not key
        ...
        if isinstance(initial_state, LayerBase):
            ...
            with rec_layer.var_creation_scope():
                var = tf_compat.v1.get_variable(
                    ...
                    initializer=tf_util.zeros_dyn_shape(shape_invariant),  # Dummy state, will not be used.
                    trainable=False,
                    collections=[tf_compat.v1.GraphKeys.GLOBAL_VARIABLES, tf_util.CollectionKeys.STATE_VARS],
                )
...
class GetRecAccumulatedOutputLayer(LayerBase):
    ...
    def get_out_data_from_opts(cls, name, sources, sub_layer, **kwargs):
        ...
        assert sub_layer in subnet.layers, "%s %r: maybe %r not with 'is_output_layer'?" % (cls, name, sub_layer)
...
class BaseChoiceLayer(LayerBase):
    ...
    def get_rec_initial_extra_outputs(
        cls, network, beam_size, search=NotSpecified, add_to_beam_scores=NotSpecified, **kwargs
    ):
        """
        :param returnn.tf.network.TFNetwork network:
        :param int beam_size:
...
class ChoiceLayer(BaseChoiceLayer):
    ...
    In case of multiple sources, this layer computes the top-k combinations of choices.
    The score of such a combination is determined by adding up the (log-space) scores
    of the choices for the individual sources.
    In this case, the 'target' parameter of the layer has to be set to a list of targets
    corresponding to the sources respectively.
    Because computing all possible combinations of source scores is costly,
    the sources are pruned beforehand using the beam
    sizes set by the 'source_beam_sizes' parameter.
    The choices made for the different sources can be accessed
    via the sublayers '<choice layer name>/out_0', '<choice layer name>/out_1' and so on.
    Note, that the way scores are combined assumes the sources to be independent.
    If you want to model a dependency,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.search_flag:
            ...
            self.search_choices = SearchChoices(owner=self, beam_size=beam_size)
            if input_type == "regression":
                ...
                if len(self.sources) > 1:
                    ...
                    if scores_in_dim is None:  # can happen if variable length
                        scores_in_dim = tf.shape(self.sources[0].output.placeholder)[
                            self.sources[0].output.feature_dim_axis
                        ]
                    pruned_labels = None
                ...
                if self.length_normalization:
                    ...
                    with tf.name_scope("length_normalization"):
                        ...
                        scores_base *= tf.where(
                            ...
                            * (
                                tf.pow(
                                    (tf.cast(t + 1, tf.float32) / tf.cast(t, tf.float32)), length_normalization_exponent
                                )
                            ),
                            tf.ones(tf.shape(end_flags)),
                        )
                ...
                if self.network.have_rec_step_info():
                    scores_in = filter_ended_scores(
                        scores_in,
                        end_flags=self.network.get_rec_step_info().get_prev_end_flag(
                            target_search_choices=base_search_choices
                        ),
                        dim=scores_in_dim,
                        batch_dim=net_batch_dim * scores_beam_in,
                    )  # (batch * beam_in, dim)
                scores_in = tf.reshape(
                    scores_in, [net_batch_dim, scores_beam_in, scores_in_dim]
                )  # (batch, beam_in, dim)
                with tf.control_dependencies(
                    [
                        ...
                        tf.Assert(
                            tf.logical_or(
                                ...
                                tf.logical_and(tf.equal(base_beam_in, scores_beam_in), tf.equal(base_beam_in, beam_in)),
                            ),
                            ["base_beam_in", base_beam_in, "scores_beam_in", scores_beam_in, "beam_in", beam_in],
                        )
                    ]
                ):
                ...
                if custom_score_combine:
                    ...
                    if random_sample_scale:
                        ...
                        scores_random_sample = -tf_compat.v1.log(
                            -tf_compat.v1.log(tf_compat.v1.random_uniform(tf.shape(scores_in), 0, 1))
                        )
                    scores_comb = optional_add(
                        optional_mul(scores_in, prob_scale),
                        optional_mul(scores_base, base_beam_score_scale),
                        optional_mul(scores_random_sample, random_sample_scale),
                    )  # (batch, beam_in, dim)
                scores_comb.set_shape(
                    (None, None, None if isinstance(scores_in_dim, tf.Tensor) else scores_in_dim)
                )  # (batch, beam_in, dim)
                ...
                if cheating:
                    cheating_gold_targets, cheating_src_beam_idx = self._get_cheating_targets_and_src_beam_idxs(
                        scores_comb
                    )
                    if isinstance(cheating, bool):
                        pass
                    elif isinstance(cheating, str):
                        assert cheating == "exclusive", "%s: invalid cheating %r" % (
                            self,
                            cheating,
                        )  # only possible variation atm
                ...
                src_beams, labels, scores = beam_search(
                    ...
                    cheating_src_beam_idx=cheating_src_beam_idx,
            ...
            if input_type == "regression":
                ...
                feedback_output = tf_compat.v1.multinomial(
                    scores_in, num_samples=1, seed=tf_util.get_random_seed()
                )  # (batch, num_samples), int64
        ...
        if add_to_beam_scores and self.sources[0].output.beam and not search and input_type != "regression":
            ...
            self.search_choices = SearchChoices(owner=self, beam_size=base_search_choices.beam_size)
            ...
            if self.network.have_rec_step_info():
                ...
                scores_in = filter_ended_scores(
                    ...
                    dim=scores_in_dim,
                    batch_dim=tf.shape(scores_in)[0],
                )  # (batch * beam_in, dim)
            ...
            self.search_choices.set_src_beams(
                expand_dims_unbroadcast(tf.range(base_search_choices.beam_size), axis=0, dim=net_batch_dim)
            )
            assert not random_sample_scale
            assert not length_normalization
            assert not custom_score_combine
            scores_comb = optional_add(
                optional_mul(scores_in_, prob_scale), optional_mul(scores_base, base_beam_score_scale)
            )  # (batch, beam_in)
            ...
            for source_index, pruned_scores_this_source in enumerate(pruned_scores):
                ...
                for axis_index in range(num_sources):
                    if axis_index != source_index:
                        expanded_pruned_scores = tf.expand_dims(
                            expanded_pruned_scores, axis_index + 1
                        )  # +1 because of batch dim
    ...
    def _get_combined_labels(self, combined_ids, src_beams, pruned_labels, beam_sizes):
        ...
        with tf.name_scope("get_combined_labels"):
            ...
            for pruned_labels_src_beam_selected_, ids_ in zip(pruned_labels_src_beam_selected, ids):
                labels_ = tf.squeeze(
                    batch_gather(pruned_labels_src_beam_selected_, tf.expand_dims(ids_, axis=-1)), axis=-1
                )
    ...
    def _get_cheating_targets_and_src_beam_idxs(self, scores):
        ...
        cheating_gold_targets = self._get_target_value(
            search_choices=None
        ).get_placeholder_as_batch_major()  # (batch,), int32
        base_search_choices = self.search_choices.src_layer.search_choices
        ...
        assert (
            other_choice_src_layer.network is self.network and other_choice_src_layer.name == "prev:%s" % self.name
        ), (
            other_choice_layer,
            other_choice_layer.search_choices,
            other_choice_src_layer,
        ...
        scores = where_bc(
            tf.expand_dims(tf.equal(base_search_choices.src_beams, prev_beam_idx), axis=-1), scores, float("-inf")
        )  # (batch,beam_in,dim)
        scores = where_bc(
            tf.equal(cheating_gold_targets[:, None, None], tf.range(dim)[None, None, :]), scores, float("-inf")
        )  # (batch,beam_in,dim)
            ...
            if search:
                if isinstance(d["explicit_search_sources"], (list, tuple)):
                    ...
                    d["explicit_search_sources"] = {
                        key: get_layer(name) for key, name in d["explicit_search_sources"].items()
                    }
            else:
                d["explicit_search_sources"] = []
    ...
    def _create_search_beam(cls, name, beam_size, sources, network):
        ...
        return SearchBeam(
            beam_size=beam_size, dependency=search_dep, name="%s%s" % (network.get_absolute_name_prefix(), name)
        )
        ...
        **kwargs,
        ...
        if target:
            out_data = cls._static_get_target_value(
                target=target, network=network, mark_data_key_as_used=False
    ...
    def get_sub_layer_out_data_from_opts(cls, layer_name, parent_layer_kwargs):
        ...
        if search:
            ...
            sub_layer_out_data.beam = cls._create_search_beam(
                name=parent_layer_name, beam_size=beam_size, sources=sources, network=network
            )
...
class DecideLayer(BaseChoiceLayer):
    ...
    def decide(cls, src, output=None, owner=None, name=None, length_normalization=False):
        ...
        src_output = tf.reshape(
            ...
            [batch_dim, beam_size] + [tf.shape(src_data.placeholder)[i] for i in range(1, src_data.batch_ndim)],
        )  # (batch, beam, [time], [dim])
        ...
        if owner:
            ...
            final_search_choices.set_beam_scores(
                tf.expand_dims(tf.gather_nd(beam_scores, indices=beam_idxs_ext), axis=1)
            )
...
class GenericAttentionLayer(AttentionBaseLayer):
    ...
    def __init__(self, weights, auto_squeeze=True, **kwargs):
        ...
        weights_data = weights_data.copy_move_axis(
            self._weights_time_axis_to_reduce(weights=weights_data, base=self.base.output), 1
        )  # (B,T,...)
        ...
        self.dot_layer = DotLayer(
            ...
            red1=weights.output.get_description_from_axis(weights_axis_to_reduce),
            red2="T",
            var1=[weights.output.get_description_from_axis(a) for a in weights_remaining_axes],
            var2="F",
        )
...
class GaussWindowAttentionLayer(AttentionBaseLayer):
    ...
    def __init__(self, window_size, std=1.0, inner_size=None, inner_size_step=0.5, **kwargs):
        """
        ...
        with tf.name_scope("idxs"):
            ...
            idxs_0 = tf.expand_dims(
                tf.range(window_size), axis=1
            )  # (beam,batch). all on cpu, but static, no round trip
        ...
        with tf.name_scope("beam"):
            ...
            batches_idxs_bc = expand_dims_unbroadcast(
                batches_idxs, axis=0, dim=tf.shape(cidxs)[0], name="batches_idxs_bc"
            )  # (beam,batch)
            idxs_exp = tf.stack(
                [cidxs, batches_idxs_bc], axis=2, name="idxs_exp"
        ...
        with tf.name_scope("gauss_window"):
            ...
            if inner_size:
                ...
                t_offs = tf.convert_to_tensor(
                    [(i * inner_size_step - inner_size / 2.0) for i in range(inner_size)]
                )  # (inner_size,)
                t_offs_bc = dimshuffle(t_offs, ("x", "x", 0))  # (batch,beam,inner_size)
                t_bc += t_offs_bc
            f_e = tf.exp(
                -((t_bc - tf.cast(idxs_tr_bc, tf.float32)) ** 2) / (2 * std_t_bc**2)
            )  # (batch,beam,inner_size)
            from math import pi, sqrt

            norm = 1.0 / (std_t_bc * sqrt(2.0 * pi))  # (batch,beam,inner_size)
...
class SelfAttentionLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        initial_state=None,
        restrict_state_to_last_seq=False,
        state_var_lengths=None,
        **kwargs,
    ):
        """
        :param int num_heads:
        ...
        q, k, v = tf.split(
            x,
            [total_key_dim // num_heads, total_key_dim // num_heads, total_value_dim // num_heads],
            axis=-1,
            name="qkv",
        )
        ...
        if input_data.time_dim_axis is not None:
            ...
            energy = where_bc(
                ...
                energy,
                -inf_value * tf.ones_like(energy),
                name="energy_masked_history",
            )
        ...
        if attention_dropout:
            weights = self.network.cond_on_train(
                fn_train=lambda: tf_util.dropout(
                    weights, keep_prob=1 - attention_dropout, seed=self.network.random.randint(2**31)
                ),
    ...
    # noinspection PyMethodOverriding
    @classmethod
    def get_rec_initial_extra_outputs_shape_invariants(
        cls, rec_layer, sources, network, num_heads, total_key_dim, out_dim=NotSpecified, n_out=NotSpecified, **kwargs
    ):
        """
...
class KenLmStateLayer(_ConcatInputLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if input_step_offset:
            new_input = tf.where(tf.greater_equal(prev_step, input_step_offset), new_input, tf.zeros_like(new_input))
        ...
        if dense_output:
            ...
            new_abs_scores = tf_ken_lm.ken_lm_abs_score_bpe_strings(
                handle=self.lm_handle, bpe_merge_symbol=bpe_merge_symbol or "", strings=next_strings
            )
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, vocab_file=None, vocab_unknown_label="UNK", dense_output=False, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
        ...
        if dense_output:
            ...
            tag = Dim(
                ...
                description="%s_ken_lm_vocab" % name,
...
class EditDistanceLayer(LayerBase):
    ...
    def __init__(self, a, b, a_spatial_dim=None, b_spatial_dim=None, **kwargs):
        ...
        y_tensor = edit_distance(
            a=a_tensor, a_len=a_ext.dim_tags[-1].dyn_size, b=b_tensor, b_len=b_ext.dim_tags[-1].dyn_size
        )
        if a_ext.batch_ndim != 2:
...
class EditDistanceTableLayer(LayerBase):
        ...
        if debug:
            ...
            if choice:
                print_out += [
                    ...
                    choice.owner.name,
                    "src_beams",
                    choice.src_beams if choice.src_beams is not None else "None",
                ]
            print_out += [
                "a_n",
                rec_step_info.step,
                "a_ended",
                rec_step_info.get_prev_end_flag(target_search_choices=self.get_search_choices()),
                "a",
                tf_util.vocab_idx_repr(source_data.placeholder, target_data),
                "b",
                tf_util.vocab_idx_repr(target_data.placeholder, target_data),
                "b_len",
                target_data.get_sequence_lengths(),
                "last_row",
                self._last_row,
                "next_row",
                self._next_row,
    ...
    @classmethod
    def get_out_data_from_opts(
        cls, name, sources, target, network, _target_layers=None, blank_idx=None, out_dim=None, **kwargs
    ):
        """
        :param str name:
        :param list[LayerBase] sources:
    ...
    def __init__(self, debug=False, blank_idx=None, **kwargs):
        ...
        if debug:
            ...
            print_out += [
                ...
                rec_step_info.get_prev_end_flag(target_search_choices=self.get_search_choices()),
                "a best",
                vocab_idx_repr(top_indices, target_data),
                top_values,
                "b",
                vocab_idx_repr(target_data.placeholder, target_data),
                "b_len",
                target_data.get_sequence_lengths(),
                "last_row",
                last_row,
...
class MaskedComputationLayer(LayerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        extra_net = self.network.make_extra_net(prefix_name="extra._internal.masked(%s)" % self.name, boundary=True)
            ...
            for key, value in sorted(self.rec_vars_outputs.items()):
                ...
                for value_, prev_value_ in zip(value_flat, prev_value_flat):
                    ...
                    if hasattr(value_, "RETURNN_masked_comp_state_transform"):
                        res.append(
                            value_.RETURNN_masked_comp_state_transform(
                                value=value_, prev_value=prev_value_, mask=mask_t
                            )
                        )
                        continue
                    check = tf.Assert(
                        ...
                        [
                            "MaskedComputation non equal state shape",
                            mask_shape,
                            tf.shape(value_),
                            tf.shape(prev_value_),
                        ],
                        summarize=10,
                    )
                        ...
                        res.append(
                            where_bc(
                                condition=tf.reshape(
                                    mask_t, [-1] + [1] * (value_.shape.ndims - 1)
                                ),  # add broadcast dims
                                x=value_,
                                y=prev_value_,
                            )
    ...
    def transform_config_dict(cls, d, network, get_layer):
        ...
        d["_layer_class"], d["_layer_desc"], d["in_spatial_dim"], d["out_spatial_dim"] = cls._create_template(
            ...
            get_layer=get_layer,
            _parent_layer_cache=parent_layer_cache,
        )
    ...
    def get_sub_layer(self, layer_name):
        ...
        over_rec_time_dim = self.network.get_inside_rec_time_dim(inside_loop=False)
        if (
            not self.masked_from
            and over_rec_time_dim == self.in_spatial_dim
            and over_rec_time_dim
            and not inside_rec_time_dim
        ):
            # We will automatically unmask again (https://github.com/rwth-i6/returnn/pull/976).
            assert self.mask
            opts = dict(name="%s(internal-unmask)" % layer.name, mask=self.mask, sources=[layer], network=layer.network)
    ...
    def __init__(self, mask, **kwargs):
        ...
        if not src_layer.output.have_time_axis():
            ...
            if self.network.is_inside_rec_layer():
                ...
                mask_out = self.mask.output
                assert (
                    mask_out.shape == (None,) and mask_out.batch_shape == (None, None) and mask_out.dtype == "bool"
...
class RHNCell(BaseRNNCell):
    ...
    def __init__(
        self, num_units, is_training=None, depth=5, dropout=0.0, dropout_seed=None, transform_bias=None, batch_size=None
    ):
        """
        :param int num_units:
    ...
    def get_input_transformed(self, x, batch_dim=None):
        ...
        bias = tf_compat.v1.get_variable(
            ...
            initializer=tf.constant_initializer([0.0] * self._num_units + [self.transform_bias] * self._num_units),
        )
        x += bias
        return x
...
class BlocksparseLSTMCell(_WrapBaseCell):
    ...
    def load_params_from_native_lstm(self, values_dict, session):
        ...
        for gate_idx in range(4):
            b_new[gate_idx * new_dim : gate_idx * new_dim + old_dim] = b_old[
                gate_idx * old_dim : (gate_idx + 1) * old_dim
            ]
...
class LayerNormVariantsLSTMCell(BaseRNNCell):
    ...
    def _norm(self, inputs, with_beta=True, add_forget_bias=False, name=None):
        ...
        s = (
            tf_compat.v1.get_variable("beta_" + name, shape=shape, initializer=tf.constant_initializer(beta_init))
            if with_beta
            else None
        )
    ...
    def _linear(self, inputs, out_dim, apply_bias=True, add_forget_bias=False, name=None):
        ...
        if apply_bias:
            ...
            bias = tf_compat.v1.get_variable(
                "bias_" + name, shape=[out_dim], initializer=tf.constant_initializer(bias_init)
            )
            out += bias
        return out
...
class TwoDLSTMLayer(LayerBase):
    ...
    def _get_cell(self, unit_opts=None):
        ...
        cell = rnn_cell_class(
            n_hidden=n_hidden,
            n_input_dim=sum(n_input_dim_parts),
    ...
    def helper_extra_outputs(cls, batch_dim, src_length, features):
        ...
        """
        return {
            "state": tf.zeros([batch_dim, 1, src_length, 5 * features]),
    ...
    def get_rec_initial_extra_outputs_shape_invariants(cls, rec_layer, n_out, sources, **kwargs):
        ...
        if sources[1].output.time_dim_axis is None:
            ...
            return {
                "state": tf.TensorShape((batch_dim, 1, src_length, 5 * n_out)),
    ...
    def _get_output_native_rec_op(self, cell):
        ...
        if cell.does_input_projection:
            ...
            weights = tf_compat.v1.get_variable(
                name="W",
                shape=(self.sources[0].output.dim, cell.n_input_dim),
                dtype=tf.float32,
                initializer=self._fwd_weights_initializer,
            )
...
class ZoneoutLSTMCell(BaseRNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        c = cond(
            self.is_training,
            lambda: (1 - self._zoneout_cell)
            * tf_compat.v1.nn.dropout(new_c - prev_c, keep_prob=(1 - self._zoneout_cell))
            + prev_c,
        ...
        h = cond(
            self.is_training,
            lambda: (1 - self._zoneout_outputs)
    ...
    def __init__(self, out_spatial_dim, axis=None, **kwargs):
        ...
        if not self.input_data.has_axis(rec_layer.time_dim_tag):  # inside loop
            ...
            if new_dim_.dyn_size_ext.placeholder is None:
                ...
                # Note: In case we have some initial state/output, this can be extended.
                new_dim_.dyn_size_ext.placeholder = (
                    tf.range(tf.math.reduce_max(rec_layer.time_dim_tag.dyn_size)) + 1
                )  # [T]
