...
class RecLayer(_ConcatInputLayer):
  ...
  def __init__(self,
    ...
    :param str forward_weights_init: see :func:`tf.util.basic.get_initializer`
    :param str recurrent_weights_init: see :func:`tf.util.basic.get_initializer`
    :param str bias_init: see :func:`tf.util.basic.get_initializer`
...
class ChoiceLayer(BaseChoiceLayer):
  ...
  def __init__(self, beam_size, keep_beams=False,
    ...
    :param bool|str cheating: if True, will always add the true target in the beam.
      if "exclusive", enables cheating_exclusive. see :func:`tf.util.basic.beam_search`.
    ...
    if self.search_flag:
      ...
      if input_type == "regression":
        ...
        # `tf.nn.top_k` is the core function performing our search. That is wrapped in `tf.util.basic.beam_search`.
  ...
  def _get_cheating_targets_and_src_beam_idxs(self, scores):
    ...
    if other_choice_layer is self:  # self from prev frame
      return cheating_gold_targets, None  # default case for tf.util.basic.beam_search
    # Different choice.
    ...
    # It also would make sense later to not add the cheating label twice (see tf.util.basic.beam_search logic);
...
class PositionalEncodingLayer(_ConcatInputLayer):
  ...
  See :func:`tf.util.basic.get_positional_encoding`.
  """
