...
def fixed_padding(inputs, kernel_size, data_format, conv_time_dim):
    ...
    if data_format == "channels_first":
        padded_inputs = tf.pad(
            inputs, [[0, 0], [0, 0], [time_pad_beg, time_pad_end], [feature_pad_beg, feature_pad_end]]
        )
    else:
        padded_inputs = tf.pad(
            inputs, [[0, 0], [time_pad_beg, time_pad_end], [feature_pad_end, feature_pad_end], [0, 0]]
        )
...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format, conv_time_dim):
    ...
    if strides > 1 or conv_time_dim:
        inputs = fixed_padding(inputs, kernel_size, data_format, conv_time_dim)

    padding = "VALID" if strides > 1 or conv_time_dim else "SAME"
...
def _building_block_v1(
    inputs, filters, training, projection_shortcut, strides, kernel_size, data_format, conv_time_dim
):
...
def _bottleneck_block_v1(
    ...
    inputs = conv2d_fixed_padding(
        inputs=inputs, filters=filters, kernel_size=1, strides=1, data_format=data_format, conv_time_dim=conv_time_dim
    )
    inputs = batch_norm(inputs, training, data_format)
    inputs = tf.nn.relu(inputs)
...
def block_layer(
    inputs, filters, bottleneck, block_fn, blocks, strides, kernel_size, training, name, data_format, conv_time_dim
):
    ...
    Args:
      ...
      data_format: The input format ('channels_last' or 'channels_first').
      conv_time_dim: Whether the conv2D operates in time_dim or window_dim.
    ...
    inputs = block_fn(inputs, filters, training, projection_shortcut, strides, kernel_size, data_format, conv_time_dim)
...
class Model(object):
    ...
    def __init__(
        ...
        Args:
          ...
          num_filters: The number of filters to use for the first block layer
            ...
            layer.
          conv_time_dim: Whether the conv2D operates in time_dim or window_dim.
          first_kernel_size: The kernel size to use for convolution.
    ...
    def __call__(self, inputs, training):
        ...
        Args:
          inputs: A Tensor representing a batch of input samples.
        ...
        with self._model_variable_scope():
            ...
            if self.conv_time_dim:
                # in case of convolving time_dime we don't need to pool it
            ...
            if self.conv_time_dim:
                inputs = tf.squeeze(inputs, axis=axes)
                if self.data_format == "channels_first":
