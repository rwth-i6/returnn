...
def load_data(config, cache_byte_size, files_config_key, **kwargs):
  ...
  elif config.is_typed(files_config_key) and isinstance(config.typed_value(files_config_key), Dataset):
    # Config contains dataset object, load it
    data = config.typed_value(files_config_key)
    data.initialize()
