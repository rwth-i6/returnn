...
class LSTMRecOpGrad(theano.sandbox.cuda.GpuOp):
  ...
  def __init__(self, fun_name, inplace, recurrent_transform):
    ...
    """
    super(LSTMRecOpGrad, self).__init__(self)
  ...
  def make_node(self, Y, H, c, y0, i, Dd, DY, *args):
    ...
    return theano.Apply(self, [Y, H, c, y0, i, Dd, DY] + args,
                        # DZ, Dc, Dy0, custom input grads, initial state var grads
                        [H.type(), c.type(), y0.type()] + custom_input_grads + initial_state_var_grads)
  ...
  def c_code(self, node, name, input_names, output_names, sub):
    (Y, H, c, y0, i, Dd, DY), remaining_inputs = input_names[:7], input_names[7:]
    ...
    (DZ, Dc, Dy0), remaining_outputs = output_names[:3], output_names[3:]
    ...
    return """
    //std::cout << "LSTMRecOpGrad called" << std::endl;
    if(!%(inplace)s)
    {
...
class LSTMRecOp(theano.sandbox.cuda.GpuOp):
  __props__ = ("inplace", "fun_name", "recurrent_transform")
  ...
  def __init__(self, fun_name, inplace, recurrent_transform):
    ...
    :type recurrent_transform: RecurrentTransform.RecurrentTransformBase
    """
    super(LSTMRecOp, self).__init__(self)
    self.inplace = inplace
    self.fun_name = fun_name
    self.recurrent_transform = recurrent_transform
    if inplace:
  ...
  def c_code(self, node, name, input_names, output_names, sub):
    ...
    Z, c, y0, i = input_names[:4]
    custom_inputs = input_names[4:]
  ...
  def grad(self, inputs, output_grads):
    (Z, c, y0, index), input_rest = inputs[:4], inputs[4:]
    ...
    all_out = self(*([Z_raw, c_raw, y0_raw, i_raw] + custom_inputs + initial_state_vars))
    ...
    all_grads = grad_op(*([Y, H, c, y0, index, Dd, DY] + custom_inputs + seq_state_var_grads))
    (DZ, Dc, Dy0), remaining_grads = all_grads[:3], all_grads[3:]
    ...
    return [DZ, Dc, Dy0, Di] + custom_input_grads + initial_state_var_grads


function_ops = {}; ":type: dict[(str,int),LSTMRecOp]"
grad_ops = {}; ":type: dict[(str,int),LSTMRecOpGrad]"
...
def register_func(recurrent_transform):
  ...
  # register op
  no_inpl = LSTMRecOp(fun_name=fn, inplace=False, recurrent_transform=recurrent_transform)
  inpl = LSTMRecOp(fun_name=fn, inplace=True, recurrent_transform=recurrent_transform)
  function_ops[key] = no_inpl

  # hack to avoid being called twice
  attr = 'LSTMRecOpInplaceOpt_%s_%i' % (fn, id(recurrent_transform))
  ...
  # the same for grad
  no_inpl = LSTMRecOpGrad(fun_name=fn, inplace=False, recurrent_transform=recurrent_transform)
  inpl = LSTMRecOpGrad(fun_name=fn, inplace=True, recurrent_transform=recurrent_transform)
  grad_ops[key] = no_inpl

  # hack to avoid being called twice
  attr = 'LSTMRecOpGradInplaceOpt_%s_%i' % (fn, id(recurrent_transform))
  if not hasattr(optdb, attr):
    opt = OpSub(no_inpl, inpl)
    optdb.register(attr, theano.gof.TopoOptimizer(opt),
                   50.0, 'fast_run', 'inplace', 'gpuarray')
